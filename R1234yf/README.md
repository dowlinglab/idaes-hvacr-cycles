...
## 2026-08-17/19 additions: reference-state/ancillary fix and PLR benchmark integration

### `R1234yf.py` / `r1234yf.json` (updated)
Two real issues found and fixed 2026-08-17 while re-checking the reference-state
offset:

1. **h_offset/s_offset=0.0 independently verified correct**, not just left
   disabled. Computed enthalpy/entropy directly from the raw EOS at the true
   saturated-liquid state (T=273.15K, found via the full Maxwell-criterion
   dome solver in `R1234yf_validation.py`, not an approximate correlation):
   h=200000.23 J/kg, s=1000.0011 J/(kg K) -- matching the IIR reference
   convention (h=200 kJ/kg, s=1.00 kJ/(kg K) at saturated liquid, 0degC) to
   ~0.0001%. The Lemmon & Akasaka (2022) EOS's own ideal-gas constants
   already bake in the IIR reference, so no rebasing is needed.
2. **A trustworthy `aux.delta_l_sat_approx`/`delta_v_sat_approx` block was
   restored to `r1234yf.json`** -- not fitted, but read directly from
   Lemmon & Akasaka (2022) Section 3 "Ancillary Equations" (Eqs. 2-3, Table 2
   coefficients, verified against the paper's own PDF). This surfaced a
   separate real bug: the paper's Table 1 triple-point values (Pt=0.4127 Pa,
   rhot_l=1577.88 kg/m^3, rhot_v=4.6552e-05 kg/m^3) were previously off by
   roughly 10x in `r1234yf.json` (Pt=0.0046 kPa, rhot_l=1550.0,
   rhot_v=0.00047) -- both now corrected to match the paper's Table 1
   directly. This ancillary block is consumed by IDAES's own
   ...
   `r1234yf_parameters.json` with no errors) and is independent of the
   h_offset/s_offset finding above.

### `r1234yf_property_package.py` (updated)
Minor follow-on edit alongside the `r1234yf.json` fixes above (4 lines
changed) -- still the superseded/stale file described above, not the current
implementation.

### `r1234yf_general_helmholtz.json`
The IDAES `general_helmholtz`-format parameter file generated by
`WriteParameters` from the corrected `r1234yf.json` (critical/triple-point
constants, ideal and residual Helmholtz coefficients, the restored ancillary
block). This is what actually gets registered with IDAES's compiled
`general_helmholtz` C++ external-function framework -- distinct from
`R1234yf.py`'s own hand-written Pyomo evaluator, which never used this file.

### `r1234yf_parameters.json`
Companion output of the same `WriteParameters` run: the `.nl`-file expression
map, variable map, and parameter block consumed by `general_helmholtz` at
solve time alongside `r1234yf_expressions_eos.nl`/`_st.nl` (the compiled
expression files themselves, not tracked in git -- see "Repo conventions").

### `vapor_compression_r1234yf.py`
A copy of `../vapor_compression.py` (the original, generic, multi-fluid
IDAES flowsheet), specialized to register and use R1234yf's own
`general_helmholtz` parameters instead of a generic `fluid_name` argument.
This is the first time this project's R1234yf model has been wired into an
actual solvable IDAES flowsheet (compressor/condenser/valve/evaporator),
rather than standalone point evaluation -- see "Known limitations" above,
now partially addressed.

### `vapor_compression_plr_r1234yf.py`
A copy of `vapor_compression_plr.py` (the PLR-only vapor compression cycle,
part-load-ratio metadata included) with two functional differences from the
original: (1) this project's own validated `r1234yf_general_helmholtz.json`
is registered automatically on import and `pure_component` is hardcoded to
R1234yf, instead of taking a generic `fluid_name` constructor argument; (2)
`general_helmholtz`'s registered component string (`"r1234yf"`, lowercase)
and CoolProp's fluid name (`"R1234yf"`, this exact capitalization) are
tracked as two separate constants, since they differ and CoolProp raises
`ValueError` if the wrong one is used. Cross-validated against `R1234yf.py`
at 10 dome points (P, rho_liq, rho_vap, H_liq, H_vap, S_liq) with exact
agreement.

### `run_plr_benchmark_r1234yf_vs_r134a.py`
The PLR benchmark runner: builds R134a (via `../vapor_compression_plr_
fixed.py`) and R1234yf (via `vapor_compression_plr_r1234yf.py`) cycles
through 100% shared setpoint/sweep/solve/output code, so the comparison is
apples-to-apples by construction. Setpoints: PLR=0.75, CD=0.13 (kept nonzero
so part-load degradation is modeled, not assumed away as a no-op), evap_sat
=-29C, condenser_approach=9C, superheat/subcool 3C floor/8C cap,
compressor_efficiency=0.80, ambient sweep 10-45C. Also reports COP_carnot
(T_cold/(T_hot-T_cold), identical for both fluids at a given ambient since it
depends only on the two saturation temperatures) against COP_full.

### `cop_vs_ambient_plr_benchmark_comparison.png`/`.pdf`
The rendered R134a-vs-R1234yf COP-vs-ambient comparison produced by
`run_plr_benchmark_r1234yf_vs_r134a.py`.
...
- This file (`README.md`) and the actual code/data/report files --
  ...
  `r1234yf_general_helmholtz.json`, `r1234yf_parameters.json`,
  `test_critical_point.py`, `R1234yf_validation.py`, `VALIDATION_REPORT.md`,
  `dome.png`, `R1234yf SI Units.pdf`, `vapor_compression_r1234yf.py`,
  `vapor_compression_plr_r1234yf.py`, `run_plr_benchmark_r1234yf_vs_
  r134a.py`, `cop_vs_ambient_plr_benchmark_comparison.png`/`.pdf` -- are
  meant to be pushed.
- `R1234yf SI Units.pdf` was found deleted from this folder (collateral
  damage from the `_merged_helmholtz_params/` merge-registry machinery, most
  likely) and restored 2026-08-19 from the byte-identical surviving copy in
  `_merged_helmholtz_params/`.
- `_merged_helmholtz_params/` is an auto-generated runtime merge-registry
  cache (built by `_register_r1234yf()`'s merge mechanism; contains copies of
  several other fluids' IDAES packages alongside R1234yf's own files) and is
  excluded from git, same category as `__pycache__/`.
- `__pycache__/`, `*.nl` files, and `_merged_helmholtz_params/` are
  gitignored per the existing/updated `.gitignore`. The 5 root-level,
  earlier PLR-benchmark files (`run_plr_cop_vs_ambient_fixed_r1234yf.py`,
  `run_plr_only_cop_vs_ambient_with_r1234yf.py`, `vapor_compression_plr_
  fixed.py`, `cop_vs_ambient_plr_fixed_comparison_r1234yf.png`, `cop_vs_
  ambient_plr_only_comparison_r1234yf.png`) at the repo root are also
  excluded, superseded by this folder's own `run_plr_benchmark_r1234yf_vs_
  r134a.py`.
