...
# ============================================================================
# THIS FILE IS A COPY of ../vapor_compression.py (the original, generic,
# multi-fluid flowsheet), specialized to R1234yf. The original file is
# NOT modified by this one -- it lives untouched one directory up.
#
# What's different from the original, and why:
#
# 1. R1234yf is not one of IDAES's own built-in general_helmholtz fluids
#    (unlike e.g. R134a/R1234ze(E), which the original file's docstring
#    mentions). This project built and validated its own R1234yf
#    Helmholtz property model this session (see BREADCRUMB.md,
#    VALIDATION_REPORT.md): `r1234yf_general_helmholtz.json` here is the
#    general_helmholtz-native parameter file (Lemmon & Akasaka 2022 EOS,
#    ancillary equations read directly from the paper's own Table 2), and
#    it was cross-checked against this project's own hand-rolled
#    R1234yf.py at 10 states spanning the full saturation dome --
#    pressure, both saturated densities, both saturated enthalpies, and
#    saturated entropy all matched to floating-point precision. So the
#    module-level setup below (`_register_r1234yf`) compiles that JSON
#    into general_helmholtz's external-function form and points
#    general_helmholtz at this folder, instead of assuming the fluid is
#    already registered.
#
# 2. The general_helmholtz component string ("r1234yf", from this JSON's
#    "comp" field) and CoolProp's fluid name ("R1234yf") are NOT
#    interchangeable -- confirmed directly: CoolProp raises
#    `ValueError: ... key [r1234yf] was not found ...` on the lowercase
#    form. The original file uses a single `fluid_name` for both
#    purposes (fine for fluids where the two happen to agree); this copy
#    tracks them as two separate constants instead.
#
# Everything else below -- the flowsheet topology, constraints,
# initialization procedure, specification logic, solve/report methods --
# is an unmodified copy of the original file's `SimpleVaporCompressionCycle`.
# ============================================================================

_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
_R1234YF_GH_JSON = os.path.join(_THIS_DIR, "r1234yf_general_helmholtz.json")
_R1234YF_GH_COMPONENT = "r1234yf"        # general_helmholtz "comp" string (this JSON's own "comp" field)
_R1234YF_COOLPROP_NAME = "R1234yf"       # CoolProp's fluid name -- different capitalization, not interchangeable


def _register_r1234yf():
    """Compile r1234yf_general_helmholtz.json into general_helmholtz's
    external-function form (if not already compiled) and register this
    folder as a parameter path, so pure_component="r1234yf" resolves.

    Idempotent / cheap to call every import: WriteParameters just
    regenerates the same .nl/parameters files each time.
    """
    WriteParameters(parameters=_R1234YF_GH_JSON).write(dry_run=False)
    set_parameter_path(_THIS_DIR)
...
class SimpleVaporCompressionCycleR1234yf:
    """Simple vapor-compression cycle, specialized to R1234yf.

    A copy of vapor_compression.py's SimpleVaporCompressionCycle, wired
    to this project's own validated R1234yf general_helmholtz model
    instead of taking a generic fluid_name. See the module docstring
    above for exactly what differs from the original file.
    """

    def __init__(self, compressor_efficiency=0.75, mode=Mode.IMPROVED_TPX):
        ''' Simple Vapor Compression Cycle for R1234yf

        Parameters:
            compressor_efficiency : float
                Isentropic efficiency of the compressor
            mode : Mode
                State variable formulation (PH, IMPROVED_TPX, ORIGINAL_TPX)

        '''

        self.fluid_name = _R1234YF_COOLPROP_NAME  # used by the CoolProp calls below
        ...
        # R1234yf property package -- this project's own validated general_helmholtz model
