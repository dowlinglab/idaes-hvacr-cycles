...
Practical implication: pressure/enthalpy outputs from a self-consistent solve (VLE bubble/dew, cycle COP, etc.) stay reliable at the ~1-2% level; liquid density itself, or any pressure computed from an externally supplied density, should not be trusted at R-515B's real composition without further correction (e.g., an updated NIST HMX.BNC/R515B.MIX file, or an empirical density correction anchored to real (T, rho) data).
