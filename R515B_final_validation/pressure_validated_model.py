...
Technical support: Claude AI and Codex
QA/Testing Responsibility: Shilpa
Creation date: (original, unrecorded); re-created 2026-08-12
Purpose of file: Pressure-validated binary Helmholtz model for the
R-1234ze(E)/R-227ea pair (R-515B), with chart-basis enthalpy support --
i.e. utilities to shift this model's own raw (arbitrary ideal-gas
reference) enthalpy onto the numerical scale used by a published P-h
chart (e.g. the Honeywell R-515B chart, reference state h=200 kJ/kg,
s=1.00 kJ/kg-K, sat. liq. at 0C), so model output can be compared
directly against that chart.
Dependencies: numpy, scipy (optional, for brentq root refinement),
density_bracketing.py, idaes.models.properties.general_helmholtz
Context reference: PROJECT_CONTEXT.md

# BREADCRUMB:
# Date: 2026-08-12
# Note: this file previously had no __main__ entry point and produced no
# output when run directly (`python3 pressure_validated_model.py`) --
# it was purely a library of function/dataclass definitions. Added a
# __main__ block below (2026-08-12) that demonstrates
# compute_props_with_chart_h() at the real Honeywell R-515B reference
# state used throughout this session's validation work
# (T=298.15 K, rho=1179.8 kg/m3, w1=0.911).
"""
...
def compute_alpha_res_pure(data, tau, delta):
    ...
    if phi == 1:
        ...
        for index in range(1, h1 + 1):
            ni = float(data["n"][str(index)])
            di = float(data["d"][str(index)])
            ti = float(data["t"][str(index)])  # BUG FIX 2026-08-12: was hardcoded to t1 for every term
            alphares_pure += ni * (delta**di) * (tau**ti)
        for index in range(h1 + 1, h2 + 1):
            ni = float(data["n"][str(index)])
            di = float(data["d"][str(index)])
        ...
        for index in range(h2 + 1, h3 + 1):
            ...
            alphares_pure += ni * (delta**di) * (tau**ti) * np.exp(-ai * ((delta - ei) ** 2) - bi * ((tau - gi) ** 2))
    elif phi == 3:
        h3 = int(hlist[2])
        for index in range(1, h1 + 1):
            ni = float(data["n"][str(index)])
        ...
        for index in range(h2 + 1, h3 + 1):
            ...
            ci = float(data["c"][str(index)])
            bi = float(data["b"][str(index)])
            alphares_pure += ni * (delta**di) * (tau**ti) * np.exp(-(delta**ci)) * np.exp(-(tau**bi))
    elif phi == 4:
        h0 = int(hlist[0])
        ...
        for j in range(1, m + 1):
            ...
            for i in range(hjm1 + 1, hj + 1):
                ...
                ti = float(data["t"][str(i)])  # BUG FIX 2026-08-12: was hardcoded to t1 for every term
                summation += ni * (delta**di) * (tau**ti)
...
def reducing_functions(data1, data2, w1, w2, betaT12, betarho12, gammaT12, gammanu12):
    ...
    nu12 = betarho12 * gammanu12 * ((nu1 ** (1 / 3) + nu2 ** (1 / 3)) ** 3) / 8.0  # BUG FIX 2026-08-12: missing /8.0 divisor per Bell 2023 paper Eq 5 (vij = betav,ij*gammav,ij*(1/8)*(vcrit_i^(1/3)+vcrit_j^(1/3))^3); confirmed against linear_model_codex.py's equivalent line
...
def default_interaction() -> InteractionParams:
    return InteractionParams(betaT12=1.001247, betarho12=0.999290, gammaT12=0.989180, gammanu12=1.001581, F12=1.0)  # betarho12 (beta_v): fixed 2026-08-12, Bell 2023 Table 2 betav,12 for R-1234ze(E)/227ea = 0.999290, not 0.99290
...
if __name__ == "__main__":
    # Demonstration entry point added 2026-08-12 -- this file previously had
    # no __main__ block and produced no output when run directly. Evaluates
    # the real Honeywell R-515B reference state used throughout this
    # session's validation work: T=298.15 K (25C), rho=1179.8 kg/m3
    # (real published saturated-liquid density), w1=0.911.
    T_demo = 298.15
    rho_demo = 1179.8
    w1_demo = 0.911
    w2_demo = 1.0 - w1_demo

    interaction_demo = default_interaction()
    chart_ref = ChartReference()  # Honeywell R-515B chart convention: T_ref=273.15K, h_ref=200 kJ/kg

    st_demo, h_chart_demo, hoff_demo, rho_ref_used_demo = compute_props_with_chart_h(
        T_demo, rho_demo, chart_ref, w1=w1_demo, w2=w2_demo, interaction=interaction_demo
    )

    print(f"T = {st_demo.T} K, rho = {st_demo.rho} kg/m3, w1 = {st_demo.w1}, w2 = {st_demo.w2}")
    print(f"tau = {st_demo.tau}, delta = {st_demo.delta}, alpha = {st_demo.alpha}")
    print(f"p = {st_demo.p} kPa")
    print(f"h (model, raw ideal-gas reference) = {st_demo.h} kJ/kg")
    print(f"h_chart_offset = {hoff_demo} kJ/kg  (reference density used: {rho_ref_used_demo} kg/m3)")
    print(f"h (shifted to chart reference convention) = {h_chart_demo} kJ/kg")
