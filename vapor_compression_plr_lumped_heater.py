...
class SimpleVaporCompressionCyclePLR:
    ...
    def _define_flowsheet(self):
        ...
        # Adding the UA variables
        self.model.fs.ua_evap = Param(initialize = 1500.0, mutable = True, units = pyunits.W/pyunits.K)
        self.model.fs.ua_cond = Param(initialize = 1800.0, mutable = True, units = pyunits.W/pyunits.K)
        
        # Adding the mdot variables
        self.model.fs.m_dot_air_evap = Param(initialize = 1.2, mutable = True, units = pyunits.kg/pyunits.s)
        self.model.fs.m_dot_air_cond = Param(initialize = 1.5, mutable = True, units = pyunits.kg/pyunits.s)
        
        # Adding the cp variables
        self.model.fs.cp_air_evap = Param(initialize = 1006.0, mutable = True, units = pyunits.J/pyunits.kg/pyunits.K)
        self.model.fs.cp_air_cond = Param(initialize = 1006.0, mutable = True, units = pyunits.J/pyunits.kg/pyunits.K)
        
        # Adding the temp variables
        self.model.fs.t_air_evap_in = Param(initialize = C_to_K - 20.0, mutable = True, units = pyunits.K)
        self.model.fs.t_air_cond_in = Param(initialize = C_to_K + 10.0, mutable = True, units = pyunits.K)
