...
def plot_envelope(
    ...
    - For mixtures, bubble/dew are distinct loci; interior is represented using
      beta-parameterized enthalpy lines.
    ...
    References
    ----------
    - Smith et al., Introduction to Chemical Engineering Thermodynamics,
      flash calculations and lever-rule relations.
    - Prausnitz et al., Molecular Thermodynamics of Fluid-Phase Equilibria,
      phase-equilibrium flash formulation.
    - MIT Unified Thermodynamics Notes, Node 69:
      https://web.mit.edu/16.unified/www/FALL/thermodynamics/notes/node69.html

    Notes on numerical stability
    ----------------------------
    - Filters to CONVERGED points to avoid plotting numerically invalid states.
    - Two-phase band is drawn from
      h(beta) = (1 - beta) h_l + beta h_v.
    """
    ...
    if len(b) and len(d):
        ...
        hd_m = hd[:m]
        pb_m = pb[:m]
        # Beta-parameterized two-phase band lines for mixture visualization.
        beta_grid = np.linspace(0.0, 1.0, 6)
        for beta in beta_grid[1:-1]:
            h_beta = np.array([mixture_two_phase_enthalpy(hl, hv, float(beta)) for hl, hv in zip(hb_m, hd_m)], dtype=float)
            ax.plot(h_beta, pb_m, color="gray", lw=0.6, alpha=0.20)
        ax.fill_betweenx(pb_m, hb_m, hd_m, alpha=0.15, label="Two-phase region (beta band)")
    ax.set_yscale("log")
...
def mixture_two_phase_enthalpy(h_l: float, h_v: float, beta: float) -> float:
    """
    Purpose
    -------
    Compute two-phase mixture enthalpy from phase enthalpies using the lever rule.
    ...
    Assumptions
    -----------
    - beta is the vapor mass fraction for a two-phase equilibrium state.
    - h_l and h_v correspond to the same pressure-temperature equilibrium pair.

    Failure modes
    -------------
    - Raises ValueError if beta is outside [0,1].
    ...
    Notes on numerical stability
    ----------------------------
    - Linear combination is numerically stable for finite enthalpies.
    """
    b = float(beta)
    if b < 0.0 or b > 1.0:
        raise ValueError(f"beta must satisfy 0 <= beta <= 1; got {beta}")
    return float((1.0 - b) * h_l + b * h_v)
