"""
1D Heat Exchanger Vapor Compression PLR Model (Copy)
...
Description:
    ...
    condenser units with IDAES HeatExchanger1D units.

Context Breadcrumb:
    HeatExchanger1D was chosen to resolve refrigerant phase-change behavior
    spatially along exchanger length (superheated, two-phase, subcooled
    regions) using a 1D discretized model instead of epsilon-NTU lumping.
...
class SimpleVaporCompressionCyclePLRHX1D:
    """Copy-only PLR cycle using HeatExchanger1D for evaporator and condenser.
    ...
    existing PLR model family; only coil unit models are swapped to 1D HX.
    """
    ...
    def __init__(
        ...
        """Construct the HX1D PLR cycle copy.
        ...
        Args:
            ...
            finite_elements: Number of 1D finite elements per HX side.
            evap_area_m2/evap_length_m/evap_u_w_m2k: Evaporator geometry/U inputs.
            cond_area_m2/cond_length_m/cond_u_w_m2k: Condenser geometry/U inputs.
        ...
        # Geometry/U inputs (initialization anchors for HX1D).
        fs.evap_area = Param(initialize=evap_area_m2, mutable=True, units=pyunits.m**2)
        fs.evap_length = Param(initialize=evap_length_m, mutable=True, units=pyunits.m)
        fs.evap_u = Param(initialize=evap_u_w_m2k, mutable=True, units=pyunits.W / pyunits.m**2 / pyunits.K)
        ...
        fs.cond_length = Param(initialize=cond_length_m, mutable=True, units=pyunits.m)
        fs.cond_u = Param(initialize=cond_u_w_m2k, mutable=True, units=pyunits.W / pyunits.m**2 / pyunits.K)
        ...
        self._define_flowsheet(finite_elements=int(finite_elements))
    ...
    def _define_flowsheet(self, finite_elements: int):
        """Build the steady-state HX1D cycle structure.

        Uses countercurrent HeatExchanger1D for both coils with explicit
        finite-element discretization and enthalpy total energy balances.
        Refrigerant-side phase behavior is handled by the Helmholtz property
        package; the HX unit model itself is agnostic and resolves enthalpy
        transport along the 1D domain.

        Args:
            finite_elements: Number of spatial elements per exchanger side.
        """
        fs = self.model.fs

        fs.evaporator = HeatExchanger1D(
            hot_side={
            ...
            cold_side={
                ...
                # Helmholtz mixed-phase state supports VLE implicitly.
                "has_phase_equilibrium": False,
                "energy_balance_type": EnergyBalanceType.enthalpyTotal,
                "has_pressure_change": False,
        ...
        self._fix_hx_geometry_and_u()
        self._fix_air_side_defaults()
        self._build_common_cycle_constraints()

    def _fix_hx_geometry_and_u(self):
        """Fix exchanger geometry and U profiles.

        Applied to both evaporator and condenser:
            - Total heat-transfer area `A`
            - Axial length `L`
            - Uniform distributed heat-transfer coefficient `U(t, x)`

        This creates a baseline UA-like behavior through `U * A`, while still
        solving full 1D enthalpy balances on both sides.
        """
        fs = self.model.fs

        fs.evaporator.area.fix(value(fs.evap_area))
        fs.evaporator.length.fix(value(fs.evap_length))
        fs.condenser.area.fix(value(fs.cond_area))
        fs.condenser.length.fix(value(fs.cond_length))

        for t in fs.time:
            for x in fs.evaporator.hot_side.length_domain:
                fs.evaporator.heat_transfer_coefficient[t, x].fix(value(fs.evap_u))
            for x in fs.condenser.hot_side.length_domain:
                fs.condenser.heat_transfer_coefficient[t, x].fix(value(fs.cond_u))

    def _fix_air_side_defaults(self):
        """Fix default air-side composition and nominal inlets for steady initialization.

        Data anchors used:
            - Norlake Split-Pak sheet (NASJ125RL4 / WL6A094SDAS):
              available capacity 9530 BTU/h; unit cooler size 43.5x15.5x18.1 in.
            - Bitzer/Buffalo Trident HX-518-1 (HVB-2-4R-4P):
              ~89.65 kW at 10 K TD (R404A table); A-dimension 1914 mm.
        """
        fs = self.model.fs
    ...
    def _build_common_cycle_constraints(self):
        ...
        fs.approach_constraint = Constraint(
            expr=fs.condenser.hot_side.properties[0, 1].temperature_sat
        ...
        fs.evap_T_lower = Constraint(
            expr=fs.evaporator.cold_side.properties[0, 1].temperature >= fs.evap_Tmin
        )
        fs.evap_T_upper = Constraint(
            expr=fs.evaporator.cold_side.properties[0, 1].temperature <= fs.evap_Tmax
        )
        fs.cond_T_lower = Constraint(
            expr=fs.condenser.hot_side.properties[0, 1].temperature >= fs.cond_Tmin
        )
        fs.cond_T_upper = Constraint(
            expr=fs.condenser.hot_side.properties[0, 1].temperature <= fs.cond_Tmax
        )
    ...
    def initialize(self, verbose=False):
        ...
        # Stepwise initializer calls with duty guesses help HX1D startup.
        try:
            fs.evaporator.initialize(outlvl=logging.WARNING, duty=10e3 * pyunits.W)
        except Exception:
            pass
        try:
        ...
        try:
            fs.condenser.initialize(outlvl=logging.WARNING, duty=12e3 * pyunits.W)
        except Exception:
            pass
        try:
    ...
    def set_specifications(
        ...
        """Apply operating specifications for steady HX1D solve.
        ...
        Key behavior:
            ...
            - Optional approach constraint links condenser saturation and ambient:
              T_cond,sat = T_ambient + approach_T
        ...
        # Lumped-equivalent closure: do not add extra condenser-sat approach equation.
        fs.approach_constraint.deactivate()
        ...
        # Equivalent assumption to non-IDAES: Tevap_sat/Tcond_sat determine pressures.
        ...
        # Basic scaling on dominant energetic terms for 1D solve robustness.
        set_scaling_factor(fs.compressor.work_mechanical[0], 1e-4)
        set_scaling_factor(fs.expansion_valve.control_volume.work[0], 1e-4)
        set_scaling_factor(fs.evaporator.hot_side.heat[0, 0], 1e-4)
        set_scaling_factor(fs.evaporator.cold_side.heat[0, 0], 1e-4)
        set_scaling_factor(fs.condenser.hot_side.heat[0, 0], 1e-4)
        set_scaling_factor(fs.condenser.cold_side.heat[0, 0], 1e-4)

        calculate_scaling_factors(self.model)
        self._seed_profiles_from_lumped(
            t_evap_sat_c=t_evap_sat_c,
            t_cond_sat_c=t_cond_sat_c,
            t_air_evap_in_c=(cold_storage_setpoint if cold_storage_setpoint is not None else -20.0),
            t_air_cond_in_c=(ambient_temperature if ambient_temperature is not None else 20.0),
        )

    def _seed_profiles_from_lumped(
        self,
        t_evap_sat_c: float,
        t_cond_sat_c: float,
        ...
        """Seed 1D refrigerant profiles from lumped model anchor states.

        This maps lumped ``h1,h2,h3,h4`` and saturation pressures to the 1D
        state variables along the refrigerant sides to provide a physically
        consistent warm-start branch for IPOPT.
        """
        fs = self.model.fs
        try:
            cfg = CycleConfig(
            ...
            rs = solve_cycle_point(
                fluid=self.cp_fluid_name,
                t_evap_sat_c=t_evap_sat_c,
                t_cond_sat_c=t_cond_sat_c,
                cfg=cfg,
                t_air_evap_in_c=t_air_evap_in_c,
                t_air_cond_in_c=t_air_cond_in_c,
            )
            d = rs.diagnostics
            h1 = float(d.get("h1_out"))
            h2 = float(d.get("h2"))
            h3 = float(d.get("h3"))
            h4 = float(d.get("h4"))
            p_low = float(rs.p_evap_pa)
            p_high = float(rs.p_cond_pa)
        except Exception:
            return
        ...
        fs.condenser.hot_side_inlet.enth_mass[0].set_value(h2)
        fs.condenser.hot_side_outlet.enth_mass[0].set_value(h3)
        fs.expansion_valve.inlet.pressure[0].set_value(p_high)
        fs.expansion_valve.outlet.pressure[0].set_value(p_low)

        # Axial profile seeds.
        for x in fs.evaporator.cold_side.length_domain:
            blk = fs.evaporator.cold_side.properties[0, x]
            h_guess = (1.0 - x) * h4 + x * h1
            h_lb = value(blk.enth_mass.lb) if blk.enth_mass.lb is not None else h_guess
            h_ub = value(blk.enth_mass.ub) if blk.enth_mass.ub is not None else h_guess
            blk.pressure.set_value(p_low)
            blk.enth_mass.set_value(min(max(h_guess, h_lb), h_ub))
        for x in fs.condenser.hot_side.length_domain:
            blk = fs.condenser.hot_side.properties[0, x]
            h_guess = (1.0 - x) * h2 + x * h3
            h_lb = value(blk.enth_mass.lb) if blk.enth_mass.lb is not None else h_guess
            h_ub = value(blk.enth_mass.ub) if blk.enth_mass.ub is not None else h_guess
            blk.pressure.set_value(p_high)
            blk.enth_mass.set_value(min(max(h_guess, h_lb), h_ub))
    ...
    def optimize_COP(self, verbose=False, initialize=True, optimize=False):
        ...
        try:
            ...
            if results.solver.termination_condition != TerminationCondition.optimal:
                # Fallback stage: temporarily release high-side target then restore.
        ...
        # Actual SH/SC as computed outputs.
        try:
            tev = value(fs.evaporator.cold_side.properties[0, 1].temperature)
            tev_sat = value(fs.evaporator.cold_side.properties[0, 1].temperature_sat)
            tcd = value(fs.condenser.hot_side.properties[0, 1].temperature)
            tcd_sat = value(fs.condenser.hot_side.properties[0, 1].temperature_sat)
