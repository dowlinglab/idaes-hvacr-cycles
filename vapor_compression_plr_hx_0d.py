"""
0-D Heat Exchanger Vapor Compression PLR Model (Copy)
...
Description:
    ...
    condenser units with IDAES HeatExchanger (0-D) units.

Context Breadcrumb:
    This variant uses IDAES 0-D HX blocks to mimic the prior
...
class SimpleVaporCompressionCyclePLRHX0D:
    """Copy-only PLR cycle using 0-D HeatExchanger for both coils.
    ...
    def __init__(
        ...
        """Construct the 0-D HX PLR cycle copy.
    ...
    def _define_flowsheet(self):
        """Build the steady-state 0-D HX cycle structure."""
        fs = self.model.fs
    ...
    def _fix_hx_0d_parameters(self):
        """Fix area and overall U for 0-D HX blocks from hot/cold UA inputs."""
        fs = self.model.fs

        fs.evaporator.area.fix(value(fs.evap_area))
        fs.condenser.area.fix(value(fs.cond_area))
        ua_evap_tot = 1.0 / (1.0 / value(fs.ua_evap_hot) + 1.0 / value(fs.ua_evap_cold))
        ua_cond_tot = 1.0 / (1.0 / value(fs.ua_cond_hot) + 1.0 / value(fs.ua_cond_cold))
        u_evap = ua_evap_tot / max(value(fs.evap_area), 1.0e-9)
        u_cond = ua_cond_tot / max(value(fs.cond_area), 1.0e-9)
        fs.evaporator.overall_heat_transfer_coefficient[0].fix(u_evap)
        fs.condenser.overall_heat_transfer_coefficient[0].fix(u_cond)
    ...
    def set_specifications(
        ...
        """Apply operating specifications for steady 0-D HX solve.
        ...
        ua_evap_tot = 1.0 / (1.0 / value(fs.ua_evap_hot) + 1.0 / value(fs.ua_evap_cold))
        ua_cond_tot = 1.0 / (1.0 / value(fs.ua_cond_hot) + 1.0 / value(fs.ua_cond_cold))
        fs.evaporator.overall_heat_transfer_coefficient[0].fix(ua_evap_tot / max(value(fs.evap_area), 1.0e-9))
        fs.condenser.overall_heat_transfer_coefficient[0].fix(ua_cond_tot / max(value(fs.cond_area), 1.0e-9))
