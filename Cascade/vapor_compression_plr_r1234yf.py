...
A copy of `vapor_compression_plr.py` (the plain IDAES PLR-only vapor
compression cycle, part-load-ratio metadata included) with exactly two
functional differences from the original:

1. This project's own validated R1234yf general_helmholtz parameter file
   (`r1234yf_general_helmholtz.json`, cross-validated this session against
   our independently-validated R1234yf.py Helmholtz model -- exact agreement
   at 10 dome points for P, rho_liq, rho_vap, H_liq, H_vap, S_liq) is
   registered automatically on import, and `pure_component` is hardcoded to
   this project's own R1234yf fluid, instead of taking a generic
   `fluid_name` constructor argument.

2. general_helmholtz's registered component string ("r1234yf", lowercase)
   and CoolProp's fluid name ("R1234yf", this exact capitalization) are
   DIFFERENT strings -- confirmed by direct test that CoolProp raises
   ValueError on the lowercase form. The original file uses a single
   `fluid_name` for both purposes; this copy tracks them as two separate
   constants instead.

Every other method (flowsheet definition, PLR/CD part-load metadata,
saturation-temperature-pinning constraints, CoolProp-based initialization,
spec-setting, optimize_COP, report_solution) is unchanged from the original.

Benchmarking setpoints (evap_sat=-29C, cond_approach=9C, PLR=0.75, CD=0.13,
superheat/subcool=3C, ambient sweep 10-45C) are supplied by the runner
script, not this file -- this file only wires in R1234yf's properties.
"""
...
_R1234YF_GH_JSON = os.path.join(_THIS_DIR, "r1234yf_general_helmholtz.json")
_R1234YF_GH_COMPONENT = "r1234yf"        # general_helmholtz "comp" string
_R1234YF_COOLPROP_NAME = "R1234yf"       # CoolProp's fluid name -- different capitalization


def _register_r1234yf():
    # WriteParameters.write() writes its compiled .nl/_parameters.json
    # output using relative filenames -- i.e. into the CURRENT WORKING
    # DIRECTORY, not into the directory of the input JSON file. Force it
    # to land in _THIS_DIR regardless of where this module is imported
    # from, by chdir'ing there temporarily.
    _prev_cwd = os.getcwd()
    try:
        os.chdir(_THIS_DIR)
        WriteParameters(parameters=_R1234YF_GH_JSON).write(dry_run=False)
    finally:
        os.chdir(_prev_cwd)

    # set_parameter_path() is destructive: internally it calls
    # clear_component_registry() and then re-scans ONLY the directory it
    # is given. Pointing it at _THIS_DIR alone would silently deregister
    # any other fluid already registered from IDAES's own built-in
    # parameter directory (e.g. R134a, R1234ze(E)) -- which matters
    # whenever this module is imported alongside those fluids in the same
    # process (e.g. a combined R134a/R1234ze(E)/R1234yf COP benchmark).
    # To register r1234yf WITHOUT breaking whatever is already
    # registered, merge the current parameter directory's contents with
    # this project's own files into one combined directory, then point
    # set_parameter_path at that merged directory instead of at _THIS_DIR
    # alone.
...
class SimpleVaporCompressionCyclePLRR1234yf:



    def __init__(self, compressor_efficiency=0.75, PLR=0.5, CD=0.25, mode=Mode.IMPROVED_TPX):
        ''' Simple Vapor Compression Cycle (R1234yf, properties hardcoded to this project's own model)
        ...
        self.fluid_name = _R1234YF_COOLPROP_NAME
        ...
        self.model.fs.properties = HelmholtzParameterBlock(pure_component=_R1234YF_GH_COMPONENT, 
                                                    state_vars=sv, 
    ...
    def _define_flowsheet(self):
        ...
        def superheating_upper_constraint(b):
            return b.control_volume.properties_out[0].temperature <= b.control_volume.properties_out[0].temperature_sat + b.superheating_max
        ...
        def subcooling_lower_constraint(b):
            return b.control_volume.properties_out[0].temperature >= b.control_volume.properties_out[0].temperature_sat - b.subcooling_max
