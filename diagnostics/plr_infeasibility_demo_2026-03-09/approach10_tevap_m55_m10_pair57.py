...
TEVAP_GRID = np.arange(-55.0, -9.0, 5.0)
...
def solve_point(ambient_c, tevap_sat_c):
    ...
    kwargs = dict(
        ...
        evaporator_temperature=(-55.0, -10.0),
...
def main():
    ...
    out_csv = f'{OUT_DIR}/approach10_tevap_m55_m10_pair57_r134a.csv'
    with open(out_csv, 'w', newline='') as f:
        w = csv.DictWriter(f, fieldnames=['ambient_C', 'tevap_sat_C', 'converged', 'cop'])
        w.writeheader(); w.writerows(rows)

    # Heatmap: COP for each ambient/tevap (infeasible masked)
    ...
    ax.set_title('Approach=10 C, Tevap_sat in [-55,-10] C, (subcool,superheat)=(5,7)')
    cbar = fig.colorbar(im, ax=ax)
    cbar.set_label('COP (infeasible points masked)')
    fig.tight_layout()

    out_png = f'{OUT_DIR}/approach10_tevap_m55_m10_pair57_r134a.png'
    out_pdf = f'{OUT_DIR}/approach10_tevap_m55_m10_pair57_r134a.pdf'
