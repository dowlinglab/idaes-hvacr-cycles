...
--- 09/03/2026 ---
Both first_principle and gcn updated to the Colon group's post-
thermoreconciliation values from spgp_r32 (4).xlsx (new Pc, Tc, pvap,
Shomate a-e for both methods). Per Shilpa's explicit direction, today's
computations use ONLY the thermoreconciled numbers -- no mixing with
the pre-reconciliation (2)/(3) values.

first_principle: Pc=49.62 bar, Tc=392.094 K, pvap=4579.217 -- confirmed
as an mmHg reading (gives a physical omega=-0.090036; the Pa reading
gives an unphysical omega=2.03). Independently verified: this data
passes the IDAES-vs-vanilla-PR cubic-EOS gate (|dZ|=4.10e-07).

gcn: Pc=63.572 bar, Tc=328.614 K, pvap=36566.042. Applying the SAME
mmHg reading convention as first_principle (same spreadsheet, same
reconciliation batch, same column -- the unit convention is a property
of the column, not of the individual method) gives omega=-0.88. This is
well outside any physically normal acentric-factor range (real fluids
run roughly -0.2 to 0.9), but is used anyway for procedural consistency
with first_principle. This is corroborating evidence of gcn's already-
flagged data-quality problem (the Colon group's own reconciled
spreadsheet notes gcn's CI is only "4.5% feasible"), not a reason to
use a different unit convention than first_principle. Treat any gcn
COP result from this data as provisional/exploratory only.

--- 09/06/2026 ---
New spreadsheet delivery (spgp_r32 (5).xlsx): first_principle and gcn's
"after thermoreconciliation" section was UPDATED (superseding the
09/03/(4).xlsx numbers), and two new sections appeared --
"multi-output raw" and "multi after recon" -- added here as
first_principle_multi / gcn_multi. "Multi-output" is not yet defined by
the collaborators; asked, not yet confirmed.
...
via the same mmHg convention: -0.209932 -- physically reasonable,
actually looks better than the 09/03 value.

gcn: Pc=62.836 bar (was 63.572), Tc=339.914 K (was 328.614),
pvap=12585.513 (was 36566.042). Shomate a-e ALSO changed this time
(unlike first_principle). omega (mmHg): -0.426564 -- still on the
negative side but a meaningful improvement over 09/03's -0.88.

first_principle_multi (new, from "multi after recon"): Pc=64.965 bar,
Tc=454.253 K, pvap=4243.056. omega (mmHg): 0.060099 -- comfortably
within the normal physical range, the best-looking omega of any
first_principle/gcn variant so far.

gcn_multi (new, from "multi after recon"): Pc=35.704 bar, Tc=385.664 K,
pvap=149.061. omega (mmHg): 1.254451 -- unphysical, same category of
problem as gcn's original 09/03 value. Treat as provisional/exploratory
only, same caveat as gcn.

All four use the mmHg-reading convention for omega_from_psat, per the
same "unit convention belongs to the column, not the method" reasoning
established 09/03.

UNCONFIRMED, pending collaborator reply: whether the Shomate a-e
coefficients are fit against T in Kelvin directly (what this file
assumes -- A=a, B=1000b, C=1e6c, D=1e9d, E=e/1e6 rescaling to this
file's t=T/1000 convention) or already fit against t=T/1000 (which
would mean NO rescaling should be applied here). Tested empirically:
evaluating first_principle's raw a-e directly with t=T/1000 (no
rescaling) gives Cp=-565,198 J/mol/K at T=298.15K -- physically absurd
-- while evaluating with T in Kelvin directly gives Cp=76.2 J/mol/K,
a sane value, matching this file's rescaled-A-E-with-t=T/1000
evaluation exactly (as it must, algebraically). This is evidence the
T-in-Kelvin assumption is right, but it's an inference from the
numbers, not a confirmation from the collaborators -- question sent to
Tina (Ching Ting Leung) today, reply pending as of this writing.

--- 09/10/2026 ---
New spreadsheet delivery (spgp_r32 (7).xlsx). Cross-checked against the
still-coded 09/06 values: the (7).xlsx workbook restates the 09/06
numbers unchanged in one block (D-H columns) and adds a genuinely new,
revised fit in a second block (I-L columns, "additional vapor
information") -- confirmed by direct cell-by-cell comparison, not
assumed. Wired in the I-L block's revised numbers here:

first_principle: Pc/Tc unchanged (60.921 bar / 355.615 K), pvap=
7853.748 (was 7409.643). Shomate a-e revised to a much larger raw
scale (a=-662.612 vs old 32.5919, etc.) -- same rescaling convention
applied (A=a, B=1000b, C=1e6c, D=1e9d, E=e/1e6), still pending Tina's
T-vs-t=T/1000 confirmation. Cp sanity check @298.15K: 84.6 J/mol/K
(was 76.2) -- still physically sane. omega (mmHg convention): -0.2352
(was -0.2099) -- similar range.

gcn: Pc/Tc unchanged (62.836 bar / 339.914 K), pvap=1670.067 (was
12585.513, a big drop). Shomate a-e also revised to a larger raw
scale. Cp sanity check @298.15K: 140.2 J/mol/K (was 142.5 under the
old fit at the same T) -- consistent. omega (mmHg convention): +0.4506
(was -0.4266) -- notable sign flip, and now much closer to real R32's
literature omega (~0.277) than any prior gcn revision. Worth flagging
to collaborators as the best-looking gcn fit yet, not just noting and
moving on.

first_principle_multi: Pc=64.94 bar (was 64.965), Tc=455.096 K (was
454.253), pvap=3392.137 (was 4243.056). Shomate a-e revised. omega
(mmHg): 0.1571 (was 0.0601) -- still comfortably in the normal range.

gcn_multi: fit FAILED this round -- (7).xlsx's I-L block has no Pc/Tc/
Shomate for gcn_multi, just pvap="failed". Kept the 09/06 fit as a
fallback (see comment above gcn_multi in METHODS_RAW) rather than
dropping the method; still provisional/unphysical (omega=1.254451),
same caveat as before.

All Cp sanity checks and omega recomputations done independently in a
plain-Python sandbox check (no IDAES needed) before wiring these
numbers in, same verification pattern as the 09/06 Shomate-convention
check above.
"""
...
# first_principle/gcn/first_principle_multi/gcn_multi: Psat(Tr=0.7)
# instead comes directly from the Colon group's own "pvap" spreadsheet
# column -- read as mmHg (confirmed physically correct for this reading,
# reversing the earlier (2)/(3)-era assumption that it was Pa). Same
# formula, different Psat source. Current (09/06) values: first_principle
# -0.209932, gcn -0.426564, first_principle_multi 0.060099 (best-behaved),
# gcn_multi 1.254451 (unphysical, provisional only).

MMHG_TO_PA = 133.322 # 1mmHg in Pa

def omega_from_psat(name, PC_pa, psat_mmHg):
    psat_pa = psat_mmHg* MMHG_TO_PA
    return -1.0 - np.log10(psat_pa/PC_pa)
...
METHODS_RAW = {
    ...
    "GCGP": {"Pc": 50.730e5, "Tc": 355.354, "omega": 0.1711,
             ...
             "F": 0.0, "G": 0.0},
    "first_principle": {"Pc": 6092100.0, "Tc": 355.615,
             "psat_mmHg": 7853.748,
             "A": -662.612, "B": 2061.4, "C": 4969.29, "D": -15122.1, "E": 8.14603,
             "F": 0.0, "G": 0.0},
    "gcn": {"Pc": 6283600.0, "Tc": 339.914,
             "psat_mmHg": 1670.067,
             "A": -4341.61, "B": 31590.1, "C": -88957.4, "D": 93746.1, "E": 43.234,
             "F": 0.0, "G": 0.0},
    "first_principle_multi":{"Pc": 6494000.0, "Tc": 455.096,
              "psat_mmHg": 3392.137,
              "A": -52.0996, "B": 960.636, "C": -3147.98, "D": 4704.94, "E": 0.818799,
              "F":0.0, "G": 0.0},
    # gcn_multi's fit FAILED in the 09/10 (spgp_r32 (7).xlsx) delivery -- no
    # Pc/Tc/Shomate coefficients came back, just pvap="failed". Falling back
    # to the last successful fit (09/06, spgp_r32 (5).xlsx) below rather than
    # dropping the method entirely. Flag to collaborators.
    "gcn_multi": {"Pc": 3570400.0, "Tc": 385.664,
                  "psat_mmHg": 149.061,
...
METHODS = {}
for name, p in METHODS_RAW.items():
    p = dict(p)
    if "psat_mmHg" in p:
        psat_mmHg = p.pop("psat_mmHg")
        p["omega"] = omega_from_psat(name, p["Pc"], psat_mmHg)
    METHODS[name] = p
...
if __name__ == "__main__":
    ...
    # Field width widened to 22 (from 6) to fit "first_principle_multi",
    # the longest method name now in METHODS -- avoids the column-merging
    # display bug seen in phase6_final_0903.py's report ("GCGPfirst_principle").
    print(f"\n{'method':>22}{'Z_idaes':>10}{'Z_vanilla':>11}{'dZ':>9}"
    ...
    for name, p in METHODS.items():
        ...
        z_idaes[name] = zi
        z_van[name] = zv
        print(f"{name:>22}{zi:10.4f}{zv:11.4f}{zi-zv:9.4f}{cpi:11.3f}{cpv:11.3f}")
