"""
# UNIFIED cycle file -- copy of vapor_compression_cubic_refstate.py, with
# Helmholtz added as a fourth `method` option ("Helmholtz", alongside
# "NIST"/"GCGP"/"SPGP"), on the SAME molar basis the cubic-PR methods
# already use. 
#
# Design: for NIST/GCGP/SPGP, EVERY line of the original cubic-PR file is
# unchanged -- same balances, same custom compressor initialization
# workaround, same dome-boundary branch-selection fixes, same everything.
# Helmholtz is added as a genuinely different branch ONLY at the specific
# points where the two property packages' APIs actually differ:
#   1. Which property block gets built (__init__).
#   2. How "saturation temperature" is spelled -- GenericParameterBlock
#      exposes temperature_bubble["Vap","Liq"]; HelmholtzParameterBlock
#      (a true single-component EoS) exposes temperature_sat directly.
#      See _Tsat() below.
#   3. Initialization: the cubic-PR methods need the elaborate custom
#      compressor/valve workarounds (_initialize_compressor_with_retry,
#      the manual valve-outlet isenthalpic solve, etc.) because IDAES's
#      built-in initialize() proved unreliable for the generic PR
#      package specifically. Helmholtz never had this problem --
#      vapor_compression_plr.py always used IDAES's plain, built-in
#      unit.initialize() calls with no workaround at all -- so
#      _initialize_helmholtz() below just does that, directly.
#   4. Composition handling: GenericParameterBlock treats R32 as one
#      "component" inside a general multi-component framework, so
#      mole_frac_comp is a real Var and creates a redundant
#      sum_mole_frac_out constraint at every unit (Task #24's fix).
#      HelmholtzParameterBlock is a genuinely pure single-species EoS --
#      mole_frac_comp there is a fixed Expression (=1), not a Var, and
#      sum_mole_frac_out doesn't exist on it at all. Every place the
#      cubic-PR code fixes/unfixes mole_frac_comp or deactivates
#      sum_mole_frac_out is guarded with `if self.method != "Helmholtz"`.
#   5. Phase selection: GenericParameterBlock's SmoothVLE needs no extra
#      constraint toggling once phase_frac is fixed (confirmed by the
#      existing "nothing to toggle here" comments below). Helmholtz's
#      TPX state carries its own eq_complementarity/eq_sat constraints
#      that DO need toggling to match -- exactly mirroring what
#      vapor_compression_plr.py's set_specifications() already does.
#
# Everything else -- the Arc structure, the COP objective/constraint, the
# pressure-bound/debug_disable_arc_pressure_eq machinery, optimize_COP(),
# report_solution() -- is 100% shared code, run identically for all four
# methods.
#
...
from phase_1_cubic_eos_validation_refstate import make_config, METHODS

# Helmholtz property package -- used only when method == "Helmholtz".
# AmountBasis.MOLE (not MASS) is the whole point of this file: it makes
# Helmholtz report flow_mol/enth_mol/entr_mol, the same molar quantities
# the cubic-PR methods already use, instead of vapor_compression_plr.py's
# flow_mass/enth_mass.
from idaes.models.properties.general_helmholtz import HelmholtzParameterBlock, StateVars, AmountBasis
...
class SimpleVaporCompressionCycle:
    ...
    def __init__(self,fluid_name, compressor_efficiency=0.75, mode=Mode.IMPROVED_TPX, method="NIST"):
        ...
        Parameters:
            ...
            method : str
                Which property package to use. Either one of the
                Colon-group R-32 critical-property/Shomate parameter sets
                ("NIST", "GCGP", "SPGP" -- see
                phase_1_cubic_eos_validation_refstate.py's METHODS dict),
                run through the generic cubic-PR package, OR
                "Helmholtz", the trusted reference EoS (CoolProp-backed),
                run through HelmholtzParameterBlock on the SAME molar
                basis as the cubic-PR methods (AmountBasis.MOLE) so all
                four methods share one basis, one cycle, one file.
                Defaults to "NIST".
        ...
        # with the generic package -- and Helmholtz is only ever built with
        # StateVars.TPX below too (never StateVars.PH), so this applies to every
        # method in this unified file, not just the cubic-PR ones.
        assert mode != Mode.PH, "Mode.PH is not supported by this unified cycle class (any method)"

        # method selects the property package: one of the three cubic-PR
        # parameter sets, run through GenericParameterBlock, or "Helmholtz",
        # run through HelmholtzParameterBlock on the same molar basis.
        assert method in METHODS or method == "Helmholtz", \
            f"method must be one of {list(METHODS.keys()) + ['Helmholtz']}, got {method!r}"
        self.method = method
        if method == "Helmholtz":
            self.model.fs.properties = HelmholtzParameterBlock(
    ...
    def _Tsat(self, state):
        """Saturation temperature of a state block, spelled the way each
        property package actually exposes it. GenericParameterBlock (the
        cubic-PR methods) has no single "temperature_sat" attribute --
        it's a phase-pair-indexed bubble-point property,
        temperature_bubble["Vap","Liq"]. HelmholtzParameterBlock is a
        genuinely pure single-component EoS, so it exposes the same
        physical quantity directly as temperature_sat, with no phase-pair
        indexing (verified against the installed IDAES source,
        general_helmholtz/helmholtz_state.py)."""
        if self.method == "Helmholtz":
            return state.temperature_sat
        return state.temperature_bubble["Vap", "Liq"]
    ...
    def _define_flowsheet(self):
        ...
        # Our flowsheet is a closed, circular loop
        # This applies equally to Helmholtz-on-moles and the cubic-PR
        # methods -- both report flow_mol as their extensive state
        # variable now (AmountBasis.MOLE for Helmholtz here, FTPx for the
        # generic package), so both create the same flow_mol_equality
        # redundancy around the closed loop.
        ...
        #
        # NEW (this file): none of this applies to Helmholtz -- its
        # mole_frac_comp is a fixed Expression (=1), not a Var, so there's no
        # independent composition variable to create this redundancy in the
        # first place, and no sum_mole_frac_out constraint exists to deactivate.
        if self.method != "Helmholtz":
        ...
        def superheating_constraint(b):
            return b.control_volume.properties_out[0].temperature >= self._Tsat(b.control_volume.properties_out[0]) + b.superheating
        ...
        def evap_sat_constraint(b):
            return self._Tsat(b.control_volume.properties_out[0]) == b.T_sat_set
        ...
        def subcooling_constraint(b):
            return b.control_volume.properties_out[0].temperature <= self._Tsat(b.control_volume.properties_out[0]) - b.subcooling
        ...
        def approach_constraint(b):
            return self._Tsat(b.control_volume.properties_out[0]) == b.ambient_T + b.approach_T
        ...
        def vapor_constraint(b):
            return b.control_volume.properties_out[0].temperature >= self._Tsat(b.control_volume.properties_out[0])
    ...
    def draw_thermodynamic_diagrams(self):
        ...
        # package. Disabled for all methods in this unified file for simplicity.
        pass
    ...
    def specify_initial_conditions(self,
        ...
        # Compute saturation pressures -- CoolProp, real-fluid data, shared
        # warm-start guesses for every method (including Helmholtz).
        ...
        # T_init guess arrays above are still used for initialization, for
        # every method in this unified file including Helmholtz.
    ...
    def _initialize_compressor_with_retry(self, compressor_state_args, verbose=False):
        ...
        Cubic-PR methods (NIST/GCGP/SPGP) ONLY -- see _initialize_helmholtz()
        for the (much simpler) Helmholtz path; this method is never called
        for method == "Helmholtz".
    ...
    def _initialize_helmholtz(self, verbose=False):
        """Plain, built-in IDAES initialization path -- used only when
        method == "Helmholtz". Mirrors vapor_compression_plr.py's
        initialize() exactly (adjusted for flow_mol/enth_mol instead of
        flow_mass/enth_mass, since this file's Helmholtz block uses
        AmountBasis.MOLE, not MASS).

        None of the elaborate cubic-PR-only workarounds above
        (_initialize_compressor_with_retry, the manual valve-outlet
        isenthalpic solve, the two-phase evaporator-inlet bound-and-seed
        recipe) are used here. Those exist because IDAES's built-in
        initialize() proved unreliable specifically for the generic PR
        package -- Helmholtz never showed those failures in this project
        (vapor_compression_plr.py always used the plain built-in path),
        so there is nothing here to work around.
        """
        p_scale = 1
    ...
    def initialize(self, verbose=False):
        ...
        if self.method == "Helmholtz":
            self._initialize_helmholtz(verbose=verbose)
            return
    ...
    def set_specifications(self,
        ...
        if self.mode == Mode.ORIGINAL_TPX:
            ...
            # Helmholtz's TPX state carries its own complementarity constraint
            # (eq_complementarity) tying phase_frac to the phase-equilibrium
            # condition -- fixing phase_frac directly conflicts with it still
            # being active, exactly mirroring what vapor_compression_plr.py's
            # set_specifications() already does for this same fix. The generic
            # PR package's SmoothVLE has no such constraint (nothing to
            # deactivate there, per the existing comments throughout this file).
            if self.method == "Helmholtz":
        ...
        if self.mode == Mode.ORIGINAL_TPX:
            ...
            # NEW (this file): the elaborate Tsat-anchored bound-tightening
            # block below (comp_isen_state/comp_out_state) exists ONLY to
            # work around the cubic-PR package's wrong-root-near-the-dome
            # branch-selection bug (extensively documented in the passes
            # below) -- Helmholtz never showed this failure anywhere in
            # this project (vapor_compression_plr.py's set_specifications
            # applies NO such bound at all). Skip it entirely for Helmholtz,
            # exactly matching what the old Helmholtz-only file already did.
            if self.method != "Helmholtz":
                ...
                # See the extensive pass-4-through-9 history in
                # vapor_compression_cubic_refstate.py for why this stays at
                # the flat pass-3-only bound -- unchanged here, cubic-PR only.
        ...
        #
        # NEW (this file): Helmholtz has no mole_frac_comp Var at all (it's
        # a fixed Expression, =1 always) -- nothing to unfix, and this
        # entire cubic-PR-only composition-anchor issue doesn't exist for it.
        if self.method != "Helmholtz":
        ...
        # Python) for the evaporator outlet fix. This bound is harmless (and
        # still applied) for Helmholtz too, since it's a plain Var bound,
        # not a composition-specific fix.
        if evap_sat_active:
        ...
        if self.mode == Mode.ORIGINAL_TPX:
            ...
            if self.method == "Helmholtz":
                # Mirrors vapor_compression_plr.py exactly: Helmholtz's TPX
                # state needs eq_complementarity deactivated and eq_sat
                # activated instead, to hand phase selection over to the
                # same smooth-VLE-style mechanism the generic package
            ...
            # else: Phase 3b -- eq_complementarity / eq_sat are Helmholtz-only;
            # the generic SmoothVLE handles the phase transition -- nothing
            # to toggle here for the cubic-PR methods.
        ...
        # NEW (this file): skipped for Helmholtz -- no such constraint exists.
        if self.method != "Helmholtz":
            for unit_name in ["evaporator", "compressor", "condenser", "expansion_valve"]:
                unit = getattr(self.model.fs, unit_name)
                unit.control_volume.properties_out[0.0].sum_mole_frac_out.deactivate()
