...
class SimpleVaporCompressionCycle:
    ...
    def specify_initial_conditions(self,
        ...
        # NOTE (2026-08-11): CP.PropsSI always returns mass-basis SI units
        # (H, S in J/kg, J/kg.K) regardless of this package's AmountBasis.
        # The dome drawn by hp_diagram/ts_diagram is now MOLE basis
        # (kJ/kmol, kJ/kmol/K -- numerically identical to J/mol, J/mol/K),
        # so h_init/S_init must be converted mass -> molar (x molar mass)
        # before plotting, then plotted RAW (no /1000) -- same reasoning
        # as the report_solution() fix above.
        molar_mass = CP.PropsSI('M', self.fluid_name)  # kg/mol
        h_init_molar = self.h_init * molar_mass  # J/kg * kg/mol = J/mol

        self.model.fs.properties.hp_diagram()
        plt.plot(h_init_molar, self.p_init/1000, 'ko')
        plt.show()
        ...
        S_init_molar = self.S_init * molar_mass  # J/kg.K * kg/mol = J/mol.K

        self.model.fs.properties.ts_diagram()
        plt.plot(S_init_molar, self.T_init, 'ko')
        plt.show()
    ...
    def set_specifications(self,
        ...
        # Set molar flowrate to 1 mol/s because we only care about thermodynamic efficiency
        # (COP is a ratio of heat_duty/work_mechanical -- the flow-rate normalization
        # cancels out of that ratio regardless of whether it's fixed in mol/s or kg/s)
    ...
    def report_solution(self):
        ...
        # NOTE (2026-08-11): h_sol/S_sol come from enth_mol()/entr_mol(),
        # which are native J/mol, J/mol/K. IDAES's dome_data() (used inside
        # hp_diagram/ts_diagram) defaults to energy_unit=kJ, mol_unit=kmol
        # for MOLE-basis packages -- kJ/kmol and kJ/kmol/K are numerically
        # IDENTICAL to J/mol and J/mol/K (1 kJ/kmol = 1000 J / 1000 mol =
        # 1 J/mol), so h_sol/S_sol must be plotted RAW here, not /1000.
        # The old /1000 was only correct back when this cycle was mass
        # basis (enth_mass in J/kg -> dome's kJ/kg needs /1000). Pressure
        # still needs /1000 (Pa -> kPa) -- that conversion doesn't depend
        # on amount basis at all.
        self.model.fs.properties.hp_diagram()
        plt.plot(h_sol, p_sol/1000, 'ko')
        add_warning()
        plt.show()
