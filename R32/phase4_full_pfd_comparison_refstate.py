...
2026-08-10 update. Identical to phase4_full_pfd_comparison.py except it
imports the cubic cycle from vapor_compression_cubic_refstate (F/G
reference-state calibration applied) instead of the original
vapor_compression_cubic. Helmholtz import is UNCHANGED -- it's the
control. Run this side by side with the original script's output: any
difference in T, P, COP, or ratioP would mean F/G is doing something
beyond shifting absolute h/s (which the math says it shouldn't). Any
difference in absolute h/s values is expected and is the whole point.
