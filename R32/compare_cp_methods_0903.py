...
METHODS = [
    ...
    {"name": "first_principle", "Pc_bar": 49.62, "Tc": 392.094,
     "psat_pa": 4579.217 * 133.322,  # (4).xlsx reconciled pvap=4579.217, confirmed
                                     # 09/03 as an mmHg reading (physical omega);
                                     # converted to Pa here since this file's
                                     # omega_from_psat() expects Pa directly, unlike
                                     # _0903.py's mmHg-native version.
     "A": 32.5919, "B": 22.3913, "C": 602.645, "D": -606.523, "E": -0.0502453},
    # gcn: UPDATED to (4).xlsx thermoreconciled values (09/03, later same day --
    # per Shilpa's direction, today's computations use ONLY the thermoreconciled
    # numbers). omega uses the SAME mmHg-reading convention as first_principle
    # (same spreadsheet/column), giving omega=-0.88 -- well outside any normal
    # physical range, consistent with the Colon group's own "4.5% feasible
    # within CI" flag on gcn. Treat gcn's curve here as provisional/exploratory.
    {"name": "gcn", "Pc_bar": 63.572, "Tc": 328.614,
     "psat_pa": 36566.042 * 133.322,  # mmHg reading, converted to Pa (same
                                       # convention as first_principle above)
