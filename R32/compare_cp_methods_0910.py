...
--- 09/10/2026 ---
New spreadsheet delivery (spgp_r32 (7).xlsx). Confirmed by direct cell
comparison that (7).xlsx restates the 09/06 numbers unchanged in one
block and adds a genuinely revised fit in a second block ("additional
vapor information") -- wired in the revised block here, matching
phase_1_cubic_eos_validation_refstate_0910.py's own 09/10 note:

first_principle: Pc/Tc unchanged, pvap=7853.748 (was 7409.643). Shomate
a-e revised to a larger raw scale. omega (mmHg): -0.235212 (was
-0.209932). Cp sanity check @298.15K: 84.6 J/mol/K -- sane.

gcn: Pc/Tc unchanged, pvap=1670.067 (was 12585.513). Shomate a-e
revised. omega (mmHg): +0.450573 (was -0.426564) -- sign flip, now the
closest to real R32's literature omega (~0.277) of any gcn revision.

first_principle_multi: Pc=64.94 (was 64.965), Tc=455.096 (was 454.253),
pvap=3392.137 (was 4243.056). omega (mmHg): 0.157137 (was 0.060099).

gcn_multi: fit FAILED this round (pvap="failed", no Pc/Tc/Shomate).
Kept the 09/06 fit as a fallback rather than dropping the method.

Also added plt.savefig() calls for both diagrams (ph_diagram_0910.png,
ts_diagram_0910.png) right before plt.show(), so a run produces files
on disk in addition to the interactive windows -- purely additive, does
not change the plots themselves.
...
METHODS = [
    ...
    # first_principle: UPDATED 09/10 to the (7).xlsx revised fit (Pc/Tc
    # unchanged, pvap and Shomate a-e both revised -- confirmed by direct
    # cell comparison that (7).xlsx's "I-L" block is genuinely new, not a
    # restatement of 09/06's (5).xlsx numbers, which are also still present
    # unchanged in (7).xlsx's "D-H" block). omega (mmHg): -0.235212 (was
    # -0.209932) -- similar range. Cp sanity check @298.15K: 84.6 J/mol/K.
    {"name": "first_principle", "Pc_bar": 60.921, "Tc": 355.615,
     "psat_pa": 7853.748 * 133.322,  # mmHg reading, converted to Pa
     "A": -662.612, "B": 2061.4, "C": 4969.29, "D": -15122.1, "E": 8.14603},
    # gcn: UPDATED 09/10 to the (7).xlsx revised fit -- Pc/Tc unchanged,
    # pvap and Shomate a-e both revised. omega (mmHg): +0.450573 (was
    # -0.426564) -- a sign flip, and now the closest to real R32's
    # literature omega (~0.277) of any gcn revision so far. Cp sanity
    # check @298.15K: 140.2 J/mol/K, consistent with the prior fit's 142.5.
    {"name": "gcn", "Pc_bar": 62.836, "Tc": 339.914,
     "psat_pa": 1670.067 * 133.322,  # mmHg reading, converted to Pa
     "A": -4341.61, "B": 31590.1, "C": -88957.4, "D": 93746.1, "E": 43.234},
    # first_principle_multi: UPDATED 09/10 to the (7).xlsx revised fit --
    # Pc/Tc/pvap and Shomate a-e all moved slightly. omega (mmHg): 0.157137
    # (was 0.060099) -- still comfortably in the normal physical range.
    {"name": "first_principle_multi", "Pc_bar": 64.94, "Tc": 455.096,
     "psat_pa": 3392.137 * 133.322,
     "A": -52.0996, "B": 960.636, "C": -3147.98, "D": 4704.94, "E": 0.818799},
    # gcn_multi: fit FAILED in the 09/10 (7).xlsx delivery -- no Pc/Tc/
    # Shomate came back, just pvap="failed". Falling back to the last
    # successful (09/06) fit below rather than dropping the method.
...
plt.tight_layout()
plt.savefig("ph_diagram_0910.png", dpi=150)
...
plt.grid(True, ls=":")
plt.tight_layout()
plt.savefig("ts_diagram_0910.png", dpi=150)
