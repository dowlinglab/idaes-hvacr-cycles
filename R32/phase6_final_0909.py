...
STILL NEEDED (not yet applied -- Shilpa is making these changes
herself): CubicCycle import needs repointing to
vapor_compression_cubic_refstate_0909 (which itself needs to import
from phase_1_cubic_eos_validation_refstate_0909 -- the file already
holding today's data). METHODS needs extending from 5 to 7 entries:
Helmholtz, NIST, GCGP, first_principle, gcn, first_principle_multi,
gcn_multi. The report table's column widths (currently 14/12/10 chars)
will also need widening -- "first_principle_multi" is 22 characters,
wider than the current fields, which will make adjacent columns run
together with no separating space once that method is added (the same
display bug seen in an earlier run's "GCGPfirst_principle" output).

Expected omega values once wired in (all via the mmHg-reading
omega_from_psat convention): first_principle -0.209932 (physical, an
improvement over 09/03), gcn -0.426564 (still negative, but better than
09/03's -0.88), first_principle_multi 0.060099 (best-behaved of any
variant so far), gcn_multi 1.254451 (unphysical -- expect this one to
struggle to converge, same category of problem as gcn's 09/03 value).
Treat gcn and gcn_multi results as provisional/exploratory, not
validated COP numbers, pending the Colon group's own confirmation of
their reconciled data (their spreadsheet already flags gcn's CI as only
"4.5% feasible").
...
couldn't run at all. That's fixed here; the import/METHODS/column-width
changes above are deliberately left for Shilpa to apply herself.
...
from vapor_compression_cubic_refstate_0909 import SimpleVaporCompressionCycle as CubicCycle, Mode as CubicMode
