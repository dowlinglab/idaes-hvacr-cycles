...
from vapor_compression_plr_ambient_bounds import SimpleVaporCompressionCyclePLR, Mode


def _build_cycle_with_fallback():
    # Try common names for R1234ze(E) depending on backend naming.
    # tuple = (IDAES pure_component name, CoolProp name override or None)
    candidates = [
        ("r1234ze", "R1234ze(E)"),
        ("R1234ze(E)", None),
        ("R1234ZEE", None),
    ...
    for idaes_name, cp_name in candidates:
        try:
            ...
            if cp_name is not None:
                # Keep IDAES key for the property package, but use a CP alias for PropsSI calls.
                cycle.fluid_name = cp_name
            return cycle, idaes_name
        except Exception as exc:
            last_err = exc
    raise RuntimeError(f"Failed to build cycle for R1234ZEE candidates: {last_err}")
...
def main():
    ...
    base_kwargs = dict(
        ...
        plr=0.75,
        cd=0.13,
        cold_storage_temperature=cold_storage_setpoint_c,
        approach_min=8.0,
        approach_max=15.0,
        soft_approach_weight=1.0,
        use_soft_approach=True,
        use_ambient_cond_sat_bounds=True,
        cond_sat_delta_min=8.0,
        cond_sat_delta_max=15.0,
    )
    ...
    for ambient_c in ambient_temps:
        ...
        run_kwargs.update(dict(ambient_temperature=float(ambient_c)))
    ...
    out_csv = "cop_vs_ambient_plr_cold_storage_r1234zee_ambient_bounds.csv"
    out_png = "cop_vs_ambient_plr_cold_storage_r1234zee_ambient_bounds.png"
    out_pdf = "cop_vs_ambient_plr_cold_storage_r1234zee_ambient_bounds.pdf"

    with open(out_csv, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow([
            "ambient_C", "cop_part_load", "cop_carnot_ref", "converged",
            "fluid_name_used", "cold_storage_setpoint_C", "evap_sat_setpoint_C", "cond_sat_target_C",
        ])
        for Ta, cop, cop_carnot, ok in zip(ambient_temps, cop_vals, cop_carnot_vals, converged_vals):
            w.writerow([
                ...
                fluid_name_used, cold_storage_setpoint_c, evap_sat_c, float(Ta + condenser_approach_c),
            ])
    ...
    ax.set_title("R1234ZEE: PLR COP vs Ambient (Ambient-Bounded Cond Sat)")
