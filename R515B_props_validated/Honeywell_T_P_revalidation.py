...
def plot_envelope(
    ...
    crit_point : dict | None [mixed units]
        ...
        vertex of BOTH the bubble and dew near-critical segments, so the
        two dashed lines visually terminate at the exact same point
        instead of leaving a gap -- this is a genuine solved closure
        (the true EOS critical point), not a cosmetic extrapolation.
    ...
    # Split each converged branch into "far from critical" (full-confidence,
    # RATIO=3.0 gate) and "near critical" (tapered gate, RATIO down to 1.05)
    # subsets, so the near-critical closing segment is visually distinguished
    # rather than drawn identically to the rest of the dome -- these points
    # are real solved solutions, just accepted under a deliberately relaxed
    # separation requirement close to Tred_mix (see _rho_separation_min_ratio).
    b_far = [r for r in b if not r.get("near_critical", False)]
    b_near = [r for r in b if r.get("near_critical", False)]
    d_far = [r for r in d if not r.get("near_critical", False)]
    d_near = [r for r in d if r.get("near_critical", False)]
    ...
    def _hp(rows: List[Dict], key: str) -> Tuple[np.ndarray, np.ndarray]:
        h = np.array([(r[key] / mw_mix) * 1e-3 for r in rows], dtype=float)
        p = np.array([r["P_Pa"] * 1e-5 for r in rows], dtype=float)
        return h, p

    hb_far, pb_far = _hp(b_far, "h_l_Jmol")
    hb_near, pb_near = _hp(b_near, "h_l_Jmol")
    hd_far, pd_far = _hp(d_far, "h_v_Jmol")
    hd_near, pd_near = _hp(d_near, "h_v_Jmol")

    # Append the solved critical point as the shared final vertex of BOTH
    # near-critical segments -- this is what actually closes the dome
    # (genuine solved coincidence, not a cosmetic patch). Only done if the
    # critical-point solve itself converged; otherwise the dome is left
    # exactly as the tapered gate produced it, with no fabricated closure.
    ...
    if have_crit:
        ...
        hb_near = np.append(hb_near, hc)
        pb_near = np.append(pb_near, pc)
        hd_near = np.append(hd_near, hc)
        pd_near = np.append(pd_near, pc)

    fig, ax = plt.subplots(figsize=(7.0, 5.5), dpi=160)
    if len(hb_far):
        ax.plot(hb_far, pb_far, lw=2.0, color="tab:blue", label="Bubble line (liq)")
    if len(hd_far):
        ax.plot(hd_far, pd_far, lw=2.0, color="tab:orange", label="Dew line (vap)")
    # Near-critical overlay: dashed, same color family, separate legend entry --
    # flags these closing-segment points as solved under the relaxed gate
    # rather than presenting them with the same confidence as the far-field dome.
    if len(hb_near):
        ax.plot(hb_near, pb_near, lw=2.0, ls="--", color="tab:blue", alpha=0.6, label="Bubble line (near-critical, tapered gate)")
    if len(hd_near):
        ax.plot(hd_near, pd_near, lw=2.0, ls="--", color="tab:orange", alpha=0.6, label="Dew line (near-critical, tapered gate)")
    # Distinct marker for the solved critical point itself, so it reads as
    # a specifically-solved physical state (not just where two lines
    # happen to end).
    if have_crit:
        ax.plot([hc], [pc], marker="*", ms=14, mfc="black", mec="black", ls="none", zorder=5, label="Critical point (solved)")
