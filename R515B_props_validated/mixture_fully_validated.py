...
ISOTHERM_VALUES_F = [-20.0, 0.0, 20.0, 40.0, 60.0, 80.0, 100.0, 120.0, 140.0, 160.0, 180.0, 200.0, 220.0]  # matches the Honeywell TDS p-h chart's printed isotherm labels (page 2)
...
def compute_isotherms_two_phase(
        d1: Dict,
        d2: Dict,
        z1: float,
        bubble_rows: List[Dict],
        dew_rows: List[Dict],
        t_values_f: List[float] = ISOTHERM_VALUES_F,        
)-> Dict[float,Dict]:
    """
    Inside the two-phase region only: an isotherm is the horizontal
    segment from (h_l,P) to (h_v,P) at a fixed T, since T and P are not
    independent inside the dome. Solves FRESH at each exact target T
    (converted from F to K), seeded from the nearest already-converged
    sweep point -- an actual EOS solve, not interpolation.
    """
    ...
    for t_f in t_values_f:
        ...
        isotherms[t_f] = {
            ...
            "h_l_Jmol": b["h_l_Jmol"],
            "h_v_Jmol": d["h_v_Jmol"],
            "rho_l_molm3": b["rho_l_molm3"],  # saturated-liquid density at this T -- seeds the liquid-side extension's root-find (see compute_isotherms_liquid_side)
            "rho_v_molm3": d["rho_v_molm3"],  # saturated-vapor density at this T -- seeds the vapor-side extension's root-find (see compute_isotherms_vapor_side)
        }
    return isotherms
...
LIQUID_EXT_P_MAX_PA = 1350.0 * 6894.757293168361  # subcooled-liquid isotherm extensions reach this pressure -- matches the Honeywell TDS p-h chart's own y-axis ceiling (1350 psia, page 2), converted directly here (not via the later PSI_TO_PA constant) to avoid a module-load-order dependency
LIQUID_EXT_N_POINTS = 25  # points per isotherm from the bubble point up to LIQUID_EXT_P_MAX_PA (inclusive) -- raised from 6 (2026-08-13: too few points made the line look kinked/faceted instead of smooth; see compute_isotherms_liquid_side's geomspace note)


def compute_isotherms_liquid_side(
        d1: Dict,
        d2: Dict,
        z1: float,
        isotherms: Dict[float, Dict],
        p_max_pa: float = LIQUID_EXT_P_MAX_PA,
        n_points: int = LIQUID_EXT_N_POINTS,
) -> Dict[float, List[Dict]]:
    """
    Subcooled-liquid extension of each two-phase isotherm (compute_isotherms_two_phase).
    Outside the dome there is only ONE phase at the fixed feed composition
    z1, so unlike the bubble/dew solve this is a single equation in a
    single unknown: find rho such that mix_state(T,rho,z1).p_pa ==
    P_target, via a bracketed root-find (brentq) seeded from the already-
    known saturated-liquid density at that T. Liquid density rises only
    modestly with pressure, so P(T,rho) is monotonically increasing over
    a modest bracket above rho_l_sat -- no least_squares/VLE machinery
    needed here at all.

    Pressure targets are sampled with np.geomspace (log-spaced), not
    np.linspace, and the plot's P-axis is log-scaled (ax.set_yscale
    ("log")) -- log-spaced samples land at visually even intervals along
    that axis. Linear spacing put nearly all the real curvature (liquid
    compressibility, hence dh/dP, is largest right at the saturation
    line) inside a single first segment, which then got drawn as one
    straight, visibly kinked line instead of a smooth curve (2026-08-13:
    "Our isotherm single phase is a kinky line, Honeywell is smooth").
    """
    ...
    for t_f, iso in isotherms.items():
        ...
        for p_target in np.geomspace(p_sat, p_max_pa, n_points)[1:]:
            ...
            try:
                ...
                # No sign change in this bracket -- P(T,rho) didn't reach
                # p_target within 1.5x the previous density; stop extending
                # this isotherm rather than guessing a wider, less-trustworthy bracket.
                break
            h_root = mix_state(d1, d2, t_k, rho_root, z1).h_jmol
            points.append({"P_Pa": float(p_target), "h_Jmol": float(h_root)})
            rho_seed = rho_root  # next bracket starts from this point, since density rises monotonically with pressure
        if len(points) > 1:
            ext[t_f] = points
    return ext


VAPOR_EXT_P_MIN_PA = 15.0 * 6894.757293168361  # superheated-vapor isotherm extensions reach DOWN to this pressure -- matches the Honeywell TDS p-h chart's own y-axis floor (15 psia, page 2), same hardcoded-conversion pattern as LIQUID_EXT_P_MAX_PA
VAPOR_EXT_N_POINTS = 40  # raised from 25 (2026-08-13: "not as smooth as Honeywell") -- log-spaced sampling, same kink-avoidance reasoning as compute_isotherms_liquid_side


def compute_isotherms_vapor_side(
        d1: Dict,
        d2: Dict,
        z1: float,
        isotherms: Dict[float, Dict],
        p_min_pa: float = VAPOR_EXT_P_MIN_PA,
        n_points: int = VAPOR_EXT_N_POINTS,
) -> Dict[float, List[Dict]]:
    """
    Superheated-vapor extension of each two-phase isotherm (mirrors
    compute_isotherms_liquid_side, opposite direction). Outside the dome
    on the vapor side there is still only ONE phase at the fixed feed
    composition z1 -- same single-equation/single-unknown root-find
    (mix_state(T,rho,z1).p_pa == P_target via bracketed brentq), seeded
    from the already-known saturated-vapor density at that T. Walks P
    DOWN from the dew point's P_sat to p_min_pa (the chart's own 15 psia
    floor), since at fixed T, dropping below P_sat is what moves a state
    from saturated vapor into superheat. Vapor density falls (not rises)
    as P drops, so each bracket searches BELOW the previous point's
    density, unlike the liquid side's upward search.

    Log-spaced (np.geomspace) from the start -- same kink-avoidance
    reasoning as compute_isotherms_liquid_side (P-axis is log-scaled;
    linear spacing concentrates real curvature into too few segments).
    """
    ext: Dict[float, List[Dict]] = {}
    for t_f, iso in isotherms.items():
        t_k = iso["T_K"]
        ...
        rho_seed = rho_v_sat
        # geomspace(p_sat, p_min_pa, ...) walks DOWN from p_sat to
        # p_min_pa; [1:] drops the duplicate first point already in
        # `points` above (mirrors the liquid side's [1:] slice).
        for p_target in np.geomspace(p_sat, p_min_pa, n_points)[1:]:
            ...
            try:
                ...
                # No sign change in this bracket -- P(T,rho) didn't reach
                # p_target within the previous density / 1.5; stop
                # extending this isotherm rather than guessing wider.
                break
            h_root = mix_state(d1, d2, t_k, rho_root, z1).h_jmol
            points.append({"P_Pa": float(p_target), "h_Jmol": float(h_root)})
            rho_seed = rho_root  # next bracket starts from this point, since density falls monotonically as pressure drops
        if len(points) > 1:
            ext[t_f] = points
    return ext


VAPOR_ONLY_ISOTHERM_VALUES_F = [240.0, 260.0, 280.0, 300.0, 320.0, 340.0, 360.0, 380.0, 400.0]  # all above the solved mixture Tc=228.0F (382.045K, 0.04% off Honeywell) -- no bubble/dew point exists at these T, single phase across the whole chart pressure range
SUPERCRIT_P_MIN_PA = 15.0 * 6894.757293168361  # matches the Honeywell TDS p-h chart's own y-axis floor (15 psia, page 2); hardcoded conversion (not the later PSI_TO_PA constant) for the same module-load-order reason as LIQUID_EXT_P_MAX_PA
SUPERCRIT_N_POINTS = 40  # raised from 25 (2026-08-13: "not as smooth as Honeywell") -- log-spaced sampling, see compute_isotherms_liquid_side's kink-avoidance note


def compute_isotherms_supercritical(
        d1: Dict,
        d2: Dict,
        z1: float,
        t_values_f: List[float] = VAPOR_ONLY_ISOTHERM_VALUES_F,
        p_min_pa: float = SUPERCRIT_P_MIN_PA,
        p_max_pa: float = LIQUID_EXT_P_MAX_PA,
        n_points: int = SUPERCRIT_N_POINTS,
) -> Dict[float, List[Dict]]:
    """
    Isotherms entirely above the solved mixture critical temperature: no
    bubble/dew point exists at these T (no phase separation above Tc), so
    unlike compute_isotherms_liquid_side there is no saturation point to
    seed the walk from. Same governing single-equation/single-unknown
    root-find as that function (mix_state(T,rho,z1).p_pa == P_target via
    bracketed brentq), but spans the ENTIRE chart pressure range
    (p_min_pa to p_max_pa) in one pass, log-spaced (np.geomspace) from
    the start -- same kink-avoidance reasoning as compute_isotherms_liquid_side.

    Walk direction is low-P -> high-P deliberately: the very first point
    has no neighboring solved density to seed from, so its bracket comes
    from an ideal-gas estimate (rho ~ P/(R*T)), which is most reliable at
    LOW pressure where real-gas departure is smallest. Every point after
    that reuses the previous point's actual solved density as its seed,
    exactly like compute_isotherms_liquid_side -- the ideal-gas estimate
    is only ever used once, to get the walk started.
    """
    R_GAS = 8.314462618  # J/(mol*K), universal gas constant -- used ONLY to size the very first bracket guess, never in the actual EOS evaluation (mix_state)
    ext: Dict[float, List[Dict]] = {}
    for t_f in t_values_f:
        t_k = _f_to_k(t_f)
        points: List[Dict] = []
        ...
        for p_target in np.geomspace(p_min_pa, p_max_pa, n_points):
            if rho_seed is None:
                # First point on this isotherm: no real neighboring
                # density exists yet. The ideal-gas estimate is only used
                # to size the bracket, not asserted as the true density.
                rho_guess = p_target / (R_GAS * t_k)
                rho_lo = max(rho_guess * 0.2, 1.0e-3)
                rho_hi = rho_guess * 5.0
            else:
                # Reuse the previous point's actual solved density, same
                # pattern as compute_isotherms_liquid_side -- wider
                # multiplicative bracket than the liquid case (0.5x-2x vs
                # 1.0x-1.5x) since supercritical density can change more
                # per pressure step, especially near the Widom line.
                rho_lo = rho_seed * 0.5
            ...
            try:
                ...
                # Bracket missed the root -- widen once before giving up
                # on this point (the ideal-gas seed especially can be off
                # by more than the liquid extension's tighter bounds).
                try:
...
def _mix_entropy_direct(d1: Dict, d2: Dict, t_k: float, rho_mol: float, x1: float) -> float:
    """
    Molar entropy [J/(mol*K)] computed DIRECTLY from the Helmholtz
    mixture model via the standard multiparameter-EOS identity
    s/R = tau*alpha_tau - alpha (e.g. [LT99]) -- NOT derived from h and
    g here. This is algebraically the same result reported by
    mixture_model_one_point.py's compute_table1_properties (there
    assembled from the split ideal+residual pieces: s0/R =
    h0/(RT) - a0_mix - 1, s_over_r = s0/R + tau*ar_tau - ar; once the
    ideal-gas chain-rule mapping tau_i=(Tc_i/Tred)*tau is applied -- which
    _mix_alpha_and_derivs already bakes into the COMBINED alpha_tau it
    returns -- tau*alpha_tau - alpha collapses to exactly that same
    split sum), computed here directly off alpha_mix/alpha_tau_mix
    themselves, per 2026-08-13 correction: "S is directly computed. Go
    back and look at the linear_model/other codes."

    REBASED 2026-08-14 to Honeywell's stated reference convention: the raw
    EOS identity below (s/R = tau*alpha_tau - alpha) carries whatever
    absolute entropy zero-point is baked into the underlying IDAES
    Helmholtz JSON parameter files -- NOT necessarily the same zero-point
    Honeywell's Solstice N15 TDS chart uses ("Reference State: h = 200
    kJ/kg, s = 1.00 kJ/kg-K; sat. liq. at 0C"). Confirmed via
    check_entropy_reference.py: raw model reads s=1.012423 kJ/(kg*K) at
    that exact reference state, a real (if small) +1.24% offset from
    Honeywell's stated 1.00. ENTROPY_REFERENCE_OFFSET_JMOLK (module-level
    constant above) is now added unconditionally so every call to this
    function returns Honeywell-referenced entropy directly -- mirrors the
    existing enthalpy chart_offset() pattern in pressure_validated_model.py,
    just applied inside the core function instead of as a separate
    caller-side wrapper, since entropy has no other consumer in this file
    besides the isentrope functions below.

    References
    ----------
    - [LT99] -- s/R = tau*(dalpha/dtau) - alpha, standard
      multiparameter-Helmholtz-EOS entropy identity.
    - mixture_model_one_point.py, compute_table1_properties -- the
      established "gold" direct-entropy reference this matches (RAW,
      pre-offset; that function has not been rebased and still returns
      the model's native zero-point).
    - Honeywell Solstice N15 (R-515B) Technical Data Sheet, p.2, reference
      state footnote.
    """
    x1c = _clip_x(x1)
    x2c = 1.0 - x1c
    alpha, alpha_tau, _ar_del = _mix_alpha_and_derivs(d1, d2, t_k, rho_mol, x1c)
    # Recompute tau independently here (rather than threading it through
    # from _mix_alpha_and_derivs), same self-contained-identity pattern
    # mix_state itself already uses.
    mw1 = mw_from_json(d1)
    mw2 = mw_from_json(d2)
    tc1 = float(d1["basic"]["Tc"])
    tc2 = float(d2["basic"]["Tc"])
...
def compute_isentropes_two_phase(
        ...
        bubble_rows: List[Dict],
        dew_rows: List[Dict],
        s_values_jmolK: List[float],
) -> Dict[float, List[Dict]]:
    """
    Two-phase portion of each isentrope. Entropy is extensive/mass-
    additive just like enthalpy, so at each T where both bubble and dew
    converged: s_l(T), s_v(T) come from the Gibbs identity at the
    ALREADY-SOLVED (T,rho_l,x1_liq)/(T,rho_v,y1_vap) states -- no new
    root-find. If a target s falls within [s_l(T), s_v(T)], the quality
    x(T) solving s_l(T)+x*(s_v(T)-s_l(T))=s_target is used to get
    h(T)=h_l(T)+x*(h_v(T)-h_l(T)), same lever rule as
    compute_quality_lines, just solved for x instead of supplied.

    As T varies across the already-converged sweep, this traces the
    isentrope's actual diagonal path through the two-phase interior
    (matches standard refrigerant p-h charts, including Honeywell's,
    per the 2026-08-13 discussion: entropy is a well-defined STATE
    property everywhere, including two-phase -- this line represents
    constant-entropy states, not a claim about which process the fluid
    is actually undergoing) -- NOT a single crossing point. Pure
    post-processing throughout, no new EOS solve.
    """
    ...
    for b, d in zip(bubble_rows, dew_rows):
        ...
        if abs(b["T_K"] - d["T_K"]) > 1.0e-6:
            continue
        t_k = b["T_K"]
        s_l = _mix_entropy_direct(d1, d2, t_k, b["rho_l_molm3"], b["x1_liq"])
        s_v = _mix_entropy_direct(d1, d2, t_k, d["rho_v_molm3"], d["y1_vap"])
        if s_v <= s_l:
            continue  # degenerate/near-critical row -- skip rather than divide by ~0
        p_pa = 0.5 * (b["P_Pa"] + d["P_Pa"])
        h_l = b["h_l_Jmol"]
        h_v = d["h_v_Jmol"]
...
def _isentrope_2eq_residual(vars_, d1: Dict, d2: Dict, z1: float, p_target: float, s_target: float) -> List[float]:
    """
    Residual for the 2-equation/2-unknown isentrope solve: unlike the
    isotherm extensions (T fixed, 1 equation for rho), an isentrope
    point outside the dome has NEITHER T nor rho fixed -- only s is --
    so each point needs [P(T,rho,z1)=P_target, s(T,rho,z1)=s_target]
    solved jointly for (T,rho) via scipy.optimize.root.

    NORMALIZED 2026-08-14: both residuals are now RELATIVE (dimensionless),
    matching the pattern already used by the original saturation solver's
    acceptance gates (r_P, r_mu). Raw residuals mixed Pa-scale (~1e5) with
    J/(mol*K)-scale (~1e2) -- a >1e5x mismatch that made hybr's numerical
    Jacobian badly conditioned, causing an immediate zero-movement failure
    even from a good seed (see diagnose_isentrope_026.py 0.26 Btu/lb-R trace,
    2026-08-14: sol0.success=False with dT=dRho=0.0000 on the very first solve).
    """
    t_k, rho = vars_
    if t_k <= 0 or rho <= 0:
        return [1.0e12, 1.0e12]  # guard against the solver stepping into an unphysical region
    st = mix_state(d1, d2, t_k, rho, z1)
    s = _mix_entropy_direct(d1, d2, t_k, rho, z1)
...
def compute_isentrope_liquid_side(
        ...
        bubble_rows: List[Dict],
        s_values_jmolK: List[float],
        p_max_pa: float = ISENTROPE_P_MAX_PA,
        n_points: int = ISENTROPE_N_POINTS,
        crit_point: Optional[Dict] = None,
) -> Dict[float, List[Dict]]:
    """
    Subcooled-liquid portion of each isentrope. At fixed T, entropy
    decreases as pressure/density increases (Maxwell relation
    ds/dP|_T = -dV/dT|_P, negative for a normal liquid), so a target
    entropy BELOW the saturated-liquid entropy s_l(T) at some T
    corresponds to a state denser than saturation at that same T.

    Anchor selection: among all CONVERGED bubble rows where
    s_l(T) > s_target (this isentrope is reachable on the subcooled
    branch at that T), picks the row with s_l(T) closest to s_target as
    the starting guess -- the best-conditioned point to seed the first
    2D solve from, since it's closest to an already-known real state.
    Warm-starts by solving AT that row's own saturation pressure first
    (should converge tightly, since the guess is already close), THEN
    walks P outward (log-spaced) up to p_max_pa, reusing each solved
    (T,rho) as the next step's seed. If no row qualifies, this isentrope
    never reaches the subcooled-liquid branch anywhere in the sweep.

    CRITICAL-POINT FALLBACK ANCHOR (added 2026-08-14): the T-grid's own
    sampled bubble rows only ever get to within ~1-2 K of the true Tc
    (run_true_vle_envelope's bubble/dew solve gets numerically fragile
    right at the pinch point, so Tmax is deliberately kept a bit short of
    Tc). That leaves a narrow band of physically valid entropies -- between
    the bubble branch's own sampled maximum and the true critical entropy
    s_c -- that no T-grid row can ever anchor, no matter how finely Tmax/n
    are tuned (entropy vs. T can be numerically steep right next to the
    critical point, so brute-force grid refinement fights the physics
    rather than closing the gap). If the ordinary T-grid search above finds
    no qualifying row AND a converged crit_point is available whose own
    entropy s_c satisfies the same "reachable" condition (s_c > s_target),
    fall back to seeding directly from crit_point's own (T_K, rho_molm3,
    P_Pa) -- a single robust, dedicated solve (solve_mixture_critical_point)
    that doesn't share the T-grid's near-critical convergence fragility.
    This only activates for targets in that narrow gap; it changes nothing
    for isentropes that already find an ordinary T-grid anchor, and it
    still correctly finds nothing for targets beyond s_c entirely (where
    even crit_point itself wouldn't satisfy s_c > s_target).
    """
    converged_bubble = [r for r in bubble_rows if r["status"] == "CONVERGED"]
    ...
    for s_target in s_values_jmolK:
        ...
        if best_row is not None:
            ...
            s_crit = _mix_entropy_direct(d1, d2, t_crit, rho_crit, x1_crit)
            if s_crit <= s_target:
                continue  # target is beyond s_c entirely -- genuinely unreachable on this branch
            t_k = t_crit
            rho_seed = rho_crit
...
def compute_isentrope_vapor_side(
        d1: Dict,
        d2: Dict,
        z1: float,
        dew_rows: List[Dict],
    ...
    compute_isentrope_liquid_side in reverse: a target entropy ABOVE the
    saturated-vapor entropy s_v(T) at some T is reachable by dropping
    below saturation pressure at that same T (isothermal expansion
    increases entropy). Same 2-equation/2-unknown scipy.optimize.root
    solve, walked in P DOWN from the anchor to p_min_pa.

    CRITICAL-POINT FALLBACK ANCHOR (added 2026-08-14): mirrors the same
    fallback added to compute_isentrope_liquid_side. The dew branch's own
    sampled minimum entropy only gets to within ~1-2 K of Tc (same T-grid
    fragility near the pinch point), leaving a narrow band of physically
    valid entropies -- between s_c and the dew branch's own sampled
    minimum -- that no T-grid row can anchor (e.g. confirmed empirically
    2026-08-14: s_c=0.3775 Btu/lb-R vs. sampled dew minimum 0.3866-0.3866
    even after pushing Tmax to 382 K / n=160, since entropy drops steeply
    in the last sliver before Tc). If the ordinary search finds no
    ...
    for s_target in s_values_jmolK:
        ...
        if best_row is not None:
            ...
            s_crit = _mix_entropy_direct(d1, d2, t_crit, rho_crit, x1_crit)
            if s_crit >= s_target:
                continue  # target is below s_c entirely -- genuinely unreachable on this branch
            t_k = t_crit
            rho_seed = rho_crit
            p_seed = crit_point["P_Pa"]
            s_anchor = s_crit
...
def plot_envelope(
    ...
    isotherms: Optional[Dict[float, Dict]] = None,
    isotherms_liquid: Optional[Dict[float, List[Dict]]] = None,
    isotherms_vapor: Optional[Dict[float, List[Dict]]] = None,
    isotherms_supercritical: Optional[Dict[float, List[Dict]]] = None,
    isentropes_two_phase: Optional[Dict[float, List[Dict]]] = None,
    isentropes_liquid: Optional[Dict[float, List[Dict]]] = None,
    isentropes_vapor: Optional[Dict[float, List[Dict]]] = None,
) -> None:
    """
    Purpose
    -------
    ...
    if quality_lines:
        ...
        for q in sorted(quality_lines.keys()):
            ...
            # NUMERIC LABEL DISABLED 2026-08-14 (user request: numbers on isentropes/quality
            # lines/isotherms made the chart hard to read). Line itself still plots.
            # ax.annotate(f"x={q:.1f}", (hq[0], pq[0]), fontsize=7, color="black", ha="right", va="center", annotation_clip=False)
    if isotherms:
        # Inside the two-phase region ONLY: an isotherm is the horizontal
        # segment from (h_l,P) to (h_v,P) at that T (T and P are not
        # independent inside the dome). Each point here came from a FRESH
        # solve at the exact target T (see compute_isotherms_two_phase),
        # not interpolation. Red to match Honeywell's own isotherm color
        # and stay legible against the black boundary/quality lines.
        for t_f in sorted(isotherms.keys()):
            iso = isotherms[t_f]
            h_l = (iso["h_l_Jmol"] / mw_mix) * 1e-3
            h_v = (iso["h_v_Jmol"] / mw_mix) * 1e-3
            p = iso["P_Pa"] * 1e-5
            ax.plot([h_l, h_v], [p, p], lw=1.0, color="red")
            # NUMERIC LABEL DISABLED 2026-08-14 (user request): line itself still plots.
            # ax.annotate(f"{t_f:.0f} F", (h_v, p), fontsize=7, color="red", ha="left", va="center", annotation_clip=False)
    if isotherms_liquid:
        # Subcooled-liquid continuation of each two-phase isotherm, OUTSIDE
        # the dome (see compute_isotherms_liquid_side). Each point is a
        # fresh single-phase root-find (brentq on P(T,rho)=P_target), not
        # interpolation. Red, matching the in-dome segments above, so each
        # isotherm reads as one continuous line (2026-08-13: "Isotherms
        # should all be in red").
        for t_f in sorted(isotherms_liquid.keys()):
            pts = isotherms_liquid[t_f]
            if len(pts) < 2:
                continue
            h_ext = np.array([(p["h_Jmol"] / mw_mix) * 1e-3 for p in pts], dtype=float)
            p_ext = np.array([p["P_Pa"] * 1e-5 for p in pts], dtype=float)
            ax.plot(h_ext, p_ext, lw=1.0, color="red")
    if isotherms_vapor:
        # Superheated-vapor continuation of each two-phase isotherm,
        # OUTSIDE the dome on the dew side (see
        # compute_isotherms_vapor_side, mirrors the liquid-side
        # extension). Red, matching every other isotherm segment.
        for t_f in sorted(isotherms_vapor.keys()):
            pts = isotherms_vapor[t_f]
            if len(pts) < 2:
                continue
            h_vap = np.array([(p["h_Jmol"] / mw_mix) * 1e-3 for p in pts], dtype=float)
            p_vap = np.array([p["P_Pa"] * 1e-5 for p in pts], dtype=float)
            ax.plot(h_vap, p_vap, lw=1.0, color="red")
    if isotherms_supercritical:
        # Above the solved mixture Tc: no bubble/dew point, single phase
        # across the whole pressure range (see compute_isotherms_supercritical).
        # Red, same as every other isotherm segment.
        for t_f in sorted(isotherms_supercritical.keys()):
            pts = isotherms_supercritical[t_f]
            if len(pts) < 2:
                continue
            h_sc = np.array([(p["h_Jmol"] / mw_mix) * 1e-3 for p in pts], dtype=float)
            p_sc = np.array([p["P_Pa"] * 1e-5 for p in pts], dtype=float)
            ax.plot(h_sc, p_sc, lw=1.0, color="red")
            # 2026-08-13: unlike the subcritical isotherms (labeled once,
            # at the dew point), supercritical isotherms have no
            # saturation point to anchor a label to at all -- Honeywell's
            # own chart labels these near the top, where the fan of lines
            # is most spread out and legible. Label at this line's last
            # (highest-P) computed point, same idea.
            # NUMERIC LABEL DISABLED 2026-08-14 (user request): line itself still plots.
            # ax.annotate(f"{t_f:.0f} F", (h_sc[-1], p_sc[-1]), fontsize=7, color="red", ha="left", va="center", annotation_clip=False)
    if isentropes_two_phase or isentropes_liquid or isentropes_vapor:
        # Constant-entropy lines (see compute_isentropes_two_phase /
        # compute_isentrope_liquid_side / compute_isentrope_vapor_side).
        # Each isentrope is drawn as up to 3 separate segments (two-phase,
        # subcooled-liquid, superheated-vapor) in the SAME color, same
        # approach as the isotherm liquid/vapor extensions, giving the
        # visual appearance of one continuous line. Blue, matching
        # Honeywell's own isentrope color (2026-08-13: "in blue as in
        # Honeywell").
        all_s_keys = set()
        for d_ in (isentropes_two_phase, isentropes_liquid, isentropes_vapor):
            if d_:
                all_s_keys.update(d_.keys())
        for s_target in sorted(all_s_keys):
            label_pt = None  # (h,p) to anchor this isentrope's single label at
            tp_pts = (isentropes_two_phase or {}).get(s_target, [])
            if len(tp_pts) >= 2:
                h_tp = np.array([(r["h_Jmol"] / mw_mix) * 1e-3 for r in tp_pts], dtype=float)
                p_tp = np.array([r["P_Pa"] * 1e-5 for r in tp_pts], dtype=float)
                order = np.argsort(p_tp)
                ax.plot(h_tp[order], p_tp[order], lw=1.0, color="blue")
                label_pt = (h_tp[order][-1], p_tp[order][-1])
            liq_pts = (isentropes_liquid or {}).get(s_target, [])
            if len(liq_pts) >= 2:
                h_liq = np.array([(r["h_Jmol"] / mw_mix) * 1e-3 for r in liq_pts], dtype=float)
                p_liq = np.array([r["P_Pa"] * 1e-5 for r in liq_pts], dtype=float)
                ax.plot(h_liq, p_liq, lw=1.0, color="blue")
                label_pt = (h_liq[-1], p_liq[-1])  # prefer the liquid branch's topmost point -- matches Honeywell's own label placement near the chart top
            vap_pts = (isentropes_vapor or {}).get(s_target, [])
            if len(vap_pts) >= 2:
                h_vap = np.array([(r["h_Jmol"] / mw_mix) * 1e-3 for r in vap_pts], dtype=float)
                p_vap = np.array([r["P_Pa"] * 1e-5 for r in vap_pts], dtype=float)
                ax.plot(h_vap, p_vap, lw=1.0, color="blue")
                if label_pt is None:
                    label_pt = (h_vap[-1], p_vap[-1])
            if label_pt is not None:
                s_btu_lbmr = s_target / BTU_LBMR_TO_JKGK / mw_mix  # J/(mol-K) -> J/(kg-K) -> Btu/(lbm-R), inverse of the _cli() conversion, final-step-only
                # NUMERIC LABEL DISABLED 2026-08-14 (user request): line itself still plots.
                # ax.annotate(f"{s_btu_lbmr:.2f} Btu/lb-R", label_pt, fontsize=7, color="blue", ha="left", va="center", annotation_clip=False)
    ax.set_yscale("log")
    ax.set_xlabel("Enthalpy [kJ/kg]")
    ax.set_ylabel("Pressure [bar]")
    ax.set_title("R515B true VLE envelope (mu-equality solve)")
    ax.grid(True, which="both", alpha=0.3)
    # Explicit box border on all 4 sides, matching the Honeywell TDS p-h
    # chart's own fully-boxed style (2026-08-13: "There needs to be a box
    # around the figure").
...
def plot_envelope_honeywell_units(
    ...
    if quality_lines:
        ...
        for q in sorted(quality_lines.keys()):
            ...
            # ax.annotate(f"x={q:.1f}", (hq[0], pq[0]), fontsize=7, color="black", ha="right", va="center", annotation_clip=False)
    if isotherms:
        # Same two-phase-only horizontal-segment logic as plot_envelope;
        # conversion to IP units happens here at the final step only.
        # Red to match Honeywell's own isotherm color.
        for t_f in sorted(isotherms.keys()):
            iso = isotherms[t_f]
            h_l_si = (iso["h_l_Jmol"] / mw_mix) * 1e-3
            h_v_si = (iso["h_v_Jmol"] / mw_mix) * 1e-3
            p_si = iso["P_Pa"]
            h_l_ip, p_ip = to_honeywell_units(h_l_si, p_si)
            h_v_ip, _ = to_honeywell_units(h_v_si, p_si)
            ax.plot([h_l_ip, h_v_ip], [p_ip, p_ip], lw=1.0, color="red")
            # NUMERIC LABEL DISABLED 2026-08-14 (user request): line itself still plots.
            # ax.annotate(f"{t_f:.0f} F", (h_v_ip, p_ip), fontsize=7, color="red", ha="left", va="center", annotation_clip=False)
    if isotherms_liquid:
        # Subcooled-liquid continuation of each isotherm, outside the dome
        # (see compute_isotherms_liquid_side). Conversion to IP units
        # happens here at the final step only. Red, matching the in-dome
        # segments (2026-08-13: "Isotherms should all be in red").
        for t_f in sorted(isotherms_liquid.keys()):
            pts = isotherms_liquid[t_f]
            if len(pts) < 2:
                continue
            h_ext_si = [(p["h_Jmol"] / mw_mix) * 1e-3 for p in pts]
            p_ext_si = [p["P_Pa"] for p in pts]
            h_ext, p_ext = _convert_all(h_ext_si, p_ext_si)
            ax.plot(h_ext, p_ext, lw=1.0, color="red")
    if isotherms_vapor:
        # Superheated-vapor continuation of each isotherm, outside the
        # dome on the dew side (see compute_isotherms_vapor_side, mirrors
        # the liquid-side extension). Conversion to IP units happens here
        # at the final step only. Red, matching every other isotherm
        # segment.
        for t_f in sorted(isotherms_vapor.keys()):
            pts = isotherms_vapor[t_f]
            if len(pts) < 2:
                continue
            h_vap_si = [(p["h_Jmol"] / mw_mix) * 1e-3 for p in pts]
            p_vap_si = [p["P_Pa"] for p in pts]
            h_vap, p_vap = _convert_all(h_vap_si, p_vap_si)
            ax.plot(h_vap, p_vap, lw=1.0, color="red")
    if isotherms_supercritical:
        # Above the solved mixture Tc: no bubble/dew point, single phase
        # across the whole pressure range (see
        # compute_isotherms_supercritical). Conversion to IP units
        # happens here at the final step only. Red, same as every other
        # isotherm segment.
        for t_f in sorted(isotherms_supercritical.keys()):
            pts = isotherms_supercritical[t_f]
            if len(pts) < 2:
                continue
            h_sc_si = [(p["h_Jmol"] / mw_mix) * 1e-3 for p in pts]
            p_sc_si = [p["P_Pa"] for p in pts]
            h_sc, p_sc = _convert_all(h_sc_si, p_sc_si)
            ax.plot(h_sc, p_sc, lw=1.0, color="red")
            # 2026-08-13: supercritical isotherms have no dew point to
            # anchor a label to -- Honeywell's own chart labels these
            # near the top of the chart, where the fan is most spread
            # out. Label at this line's last (highest-P) computed point.
            # NUMERIC LABEL DISABLED 2026-08-14 (user request): line itself still plots.
            # ax.annotate(f"{t_f:.0f} F", (h_sc[-1], p_sc[-1]), fontsize=7, color="red", ha="left", va="center", annotation_clip=False)
    if isentropes_two_phase or isentropes_liquid or isentropes_vapor:
        # Constant-entropy lines, same 3-segment approach as plot_envelope
        # above. Conversion to IP units happens here at the final step
        # only, via the same _convert_all helper used elsewhere in this
        # function. Blue, matching Honeywell's own isentrope color.
        all_s_keys = set()
        for d_ in (isentropes_two_phase, isentropes_liquid, isentropes_vapor):
            if d_:
                all_s_keys.update(d_.keys())
        for s_target in sorted(all_s_keys):
            ...
            tp_pts = (isentropes_two_phase or {}).get(s_target, [])
            if len(tp_pts) >= 2:
                h_tp_si = [(r["h_Jmol"] / mw_mix) * 1e-3 for r in tp_pts]
                p_tp_si = [r["P_Pa"] for r in tp_pts]
                h_tp, p_tp = _convert_all(h_tp_si, p_tp_si)
                order = np.argsort(p_tp)
                ax.plot(h_tp[order], p_tp[order], lw=1.0, color="blue")
                label_pt = (h_tp[order][-1], p_tp[order][-1])
            liq_pts = (isentropes_liquid or {}).get(s_target, [])
            if len(liq_pts) >= 2:
                h_liq_si = [(r["h_Jmol"] / mw_mix) * 1e-3 for r in liq_pts]
                p_liq_si = [r["P_Pa"] for r in liq_pts]
                h_liq, p_liq = _convert_all(h_liq_si, p_liq_si)
                ax.plot(h_liq, p_liq, lw=1.0, color="blue")
                label_pt = (h_liq[-1], p_liq[-1])
            vap_pts = (isentropes_vapor or {}).get(s_target, [])
            if len(vap_pts) >= 2:
                h_vap_si = [(r["h_Jmol"] / mw_mix) * 1e-3 for r in vap_pts]
                p_vap_si = [r["P_Pa"] for r in vap_pts]
                h_vap, p_vap = _convert_all(h_vap_si, p_vap_si)
                ax.plot(h_vap, p_vap, lw=1.0, color="blue")
                if label_pt is None:
                    label_pt = (h_vap[-1], p_vap[-1])
            if label_pt is not None:
                s_btu_lbmr = s_target / BTU_LBMR_TO_JKGK / mw_mix
                # NUMERIC LABEL DISABLED 2026-08-14 (user request): line itself still plots.
                # ax.annotate(f"{s_btu_lbmr:.2f} Btu/lb-R", label_pt, fontsize=7, color="blue", ha="left", va="center", annotation_clip=False)
    ax.set_yscale("log")
    # Axis label text and tick values below match the Honeywell TDS p-h
    # chart's own convention exactly (page 2, confirmed against the
    # full-resolution reference image, 2026-08-13: "axes labels must
    # follow Honeywell convention"): parenthesized units, and the
    # chart's own non-uniform, explicitly-labeled log-scale gridlines
    # (NOT matplotlib's default log-axis tick spacing).
    ...
    ax.grid(True, which="both", alpha=0.3)
    # Match the Honeywell TDS p-h chart's own axis extents (page 2), per
    # explicit user instruction (2026-08-13): pressure [15, 1400] psia,
    # enthalpy [70, 230] Btu/lbm. Set after set_yscale("log") -- limits on
    # a log axis still take plain data values, not log-transformed ones.
    ax.set_xlim(70.0, 230.0)
    ax.set_ylim(15.0, 1400.0)
    honeywell_p_ticks = [15, 30, 45, 60, 75, 90, 105, 120, 135, 150, 300, 450, 600, 750, 900, 1050, 1200, 1350]  # Honeywell's own printed y-axis gridline values, read directly off the reference chart -- not evenly log-spaced (denser below 150 psia)
    ax.set_yticks(honeywell_p_ticks)
    ax.set_yticklabels([str(v) for v in honeywell_p_ticks])
    ax.set_yticks([], minor=True)  # suppress matplotlib's default log-axis minor ticks, which don't match Honeywell's explicit gridline set
    honeywell_h_ticks = [70, 90, 110, 130, 150, 170, 190, 210, 230]  # Honeywell's own printed x-axis gridline values (20 Btu/lbm steps), read directly off the reference chart
    ax.set_xticks(honeywell_h_ticks)
    # Explicit box border on all 4 sides, matching the Honeywell TDS p-h
    # chart's own fully-boxed style (2026-08-13: "There needs to be a box
    # around the figure").
    for spine in ax.spines.values():
...
def _cli() -> None:
    ...
    isotherms = compute_isotherms_two_phase(d1, d2, z1, bubble_rows, dew_rows)  # FRESH EOS solves at exact round-F temperatures (ISOTHERM_VALUES_F), two-phase region only
    isotherms_liquid = compute_isotherms_liquid_side(d1, d2, z1, isotherms)  # subcooled-liquid continuation outside the dome, up to the Honeywell chart's own 1350 psia ceiling
